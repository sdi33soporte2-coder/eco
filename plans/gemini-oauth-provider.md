...
## Token Lifecycle
- Store at `~/.eco/gemini_oauth.json` (NOT sharing with `~/.gemini/oauth_creds.json`)
...
2. `eco_cli/auth.py` — Add ProviderConfig for "gemini" with auth_type="oauth_google"
3. `eco_cli/models.py` — Add Gemini model catalog
4. `eco_cli/runtime_provider.py` — Add gemini branch (read OAuth token, build OpenAI client)
5. `eco_cli/main.py` — Add `_model_flow_gemini()`, add to provider choices
6. `eco_cli/setup.py` — Add gemini auth flow (trigger browser OAuth)
...
- eco-agent Copilot OAuth: `eco_cli/main.py` `_copilot_device_flow()` (different flow type but same lifecycle pattern)
