...
def render_skill_page(
    ...
    eco_meta = (fm.get("metadata") or {}).get("eco") or {}
    tags = eco_meta.get("tags") or []
    related = eco_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `eco skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
    ...
    return (
        ...
        "\n"
        ":::info\n"
        "The following is the complete skill definition that ECO loads when this skill is triggered. This is what the agent sees as instructions when the skill is active.\n"
        ":::\n"
        "\n"
        f"{body_clean}\n"
    )
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with ECO Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "ECO ships with a large built-in skill library copied into `~/.eco/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "ECO also syncs bundled skills on `eco update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.eco/skills/` tree, it is still shipped with ECO; restore it with `eco skills reset <name> --restore`.",
        "",
...
def build_catalog_md_optional(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Official optional skills shipped with eco — install via eco skills install official/<category>/<skill>"',
        "---",
        "",
        "# Optional Skills Catalog",
        "",
        "Optional skills ship with eco under `optional-skills/` but are **not active by default**. Install them explicitly:",
        "",
        "```bash",
        "eco skills install official/<category>/<skill>",
        "```",
        "",
        "For example:",
        "",
        "```bash",
        "eco skills install official/blockchain/solana",
        "eco skills install official/mlops/flash-attention",
        "```",
        "",
        ...
        "",
        "```bash",
        "eco skills uninstall <skill-name>",
        "```",
        "",
    ]
    for category in sorted(by_cat):
