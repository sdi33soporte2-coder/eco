...
- smoke-test a server with the FastMCP CLI before wiring it into ECO or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to ECO. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.eco/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.eco/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is ECO integration, either:

- configure the server in `~/.eco/config.yaml` using the `native-mcp` skill, or
...
### ECO cannot see the deployed server

The server-building part may be correct while the ECO config is not. Load the `native-mcp` skill and configure the server in `~/.eco/config.yaml`, then restart ECO.
