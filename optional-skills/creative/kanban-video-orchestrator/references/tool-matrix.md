...
Maps each role archetype to the ECO skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public eco-agent
...
## ECO skills relevant to video production

### Visual / rendering skills (`eco-agent/skills/creative/`)
...
### Design / pre-production skills (`eco-agent/skills/creative/`)
...
### Audio / media skills (`eco-agent/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`eco-agent/skills/devops/`)
...
These are **not** ECO skills but external CLIs / APIs that profiles invoke.
...
## Built-in ECO tools for media review

These are native ECO tools — not invoked via terminal but through their own
...
key is present in `~/.eco/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every ECO profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `~/.eco/.env` → environment variable.
...
When a new ECO-public video skill ships:
