...
eco kanban watch --tenant <project-slug>

# Snapshot of the board
eco kanban list --tenant <project-slug>
eco kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
eco kanban stats --tenant <project-slug>

# Visual dashboard (browser)
eco dashboard

# Inspect a specific task (includes comments + events)
eco kanban show <task-id>

# Follow a single task's event stream
eco kanban tail <task-id>
```

Verify available subcommands with `eco kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `eco kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `eco kanban block <id>` then `eco kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `eco kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `eco kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `eco profile list` (profile exists?), `eco status` (gateway/dispatcher up?), and verify tenant. |
...
NEW_TASK_ID=$(eco kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
eco kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
eco kanban block <task-id>
eco kanban archive <task-id>

# Diagnose what happened
eco kanban show <task-id>      # task body, comments, recent events
eco kanban tail <task-id>      # follow the live event stream
eco kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `eco kanban create` for the director
...
while true; do
    clear
    eco kanban list --tenant <slug>
    echo "---"
    eco kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `eco kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `eco profile list` and start
