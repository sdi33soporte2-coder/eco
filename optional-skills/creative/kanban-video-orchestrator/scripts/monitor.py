#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `eco kanban list` and
...
def eco_available() -> bool:
    return shutil.which("eco") is not None


def kanban_list(tenant: str) -> list[dict]:
    """Returns parsed task rows. Falls back to plain stdout parsing if JSON
    output isn't supported by the installed eco CLI."""
    try:
        out = subprocess.run(
            ["eco", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `eco kanban list`
    out = subprocess.run(
        ["eco", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, check=False,
    )
    rows = []
...
def main():
    ...
    if not eco_available():
        print("ERROR: 'eco' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
