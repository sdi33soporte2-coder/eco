...
version: 0.1.0
author: Matt Maximo (MattMaximo), ECO Agent
license: MIT
platforms: [linux, macos, windows]
metadata:
  eco:
    tags: [Coding-Agent, Grok, xAI, Code-Review, Refactoring, Automation]
    related_skills: [codex, claude-code, eco-agent]
---

# Grok Build CLI — ECO Orchestration Guide

Delegate coding tasks to [Grok Build](https://docs.x.ai/build/overview) (xAI's
autonomous coding agent CLI, the `grok` command) via the ECO terminal. Grok
...
1. Prepare stable input files first with ECO tools (`read_file`,
...
2. **Don't conflate ECO' xAI auth with the `grok` CLI's auth.** ECO'
...
7. **Verify auth before relying on it** — check `~/.grok/auth.json` or run a
   cheap `grok -p "Say ok."` smoke test; don't assume ECO' xAI auth carries
   over.
