...
version: 0.1.0
author: Tim Koepsel (xzessmedia), ECO Agent
license: MIT
platforms: [linux, macos]
metadata:
  eco:
    tags: [Coding-Agent, OpenHands, Model-Agnostic, LiteLLM]
    related_skills: [claude-code, codex, opencode, eco-agent]
---
...
This skill is the headless-mode wrapper for batch / one-shot delegation. The interactive textual UI is not used from ECO.
...
For Claude-native, prefer `claude-code`. For OpenAI-native, prefer `codex`. For ECO-native subagents, use `delegate_task`.
...
- Sibling skills: `claude-code` (Anthropic-only), `codex` (OpenAI-only), `opencode` (multi-provider via OpenCode), `eco-agent` (ECO subagents via `delegate_task`).
