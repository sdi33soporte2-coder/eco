...
# Honcho Memory for ECO

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every ECO profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/eco#running-honcho-locally-with-eco

### Verify

```bash
eco honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this ECO profile's AI peer
...
Honcho models conversations as interactions between **peers**. ECO creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this ECO instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each ECO run |
| `global` | Single session across all directories |

Manual override: `eco honcho map my-project-name`
...
Each ECO profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
```bash
eco profile create coder --clone
# creates host block eco.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `eco.coder` host block in `honcho.json`
...
```bash
eco honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for ECO when Honcho memory is active.
...
Run `eco honcho setup`. Ensure `memory.provider: honcho` is in `~/.eco/config.yaml`.

### Memory not persisting across sessions
Check `eco honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `eco profile create <name> --clone`. For existing profiles: `eco honcho sync`.
...
| Command | Description |
|---------|-------------|
| `eco honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `eco honcho status` | Show resolved config, connection test, peer info for active profile |
| `eco honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `eco honcho disable` | Disable Honcho for the active profile |
| `eco honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `eco honcho peers` | Show peer identities across all profiles |
| `eco honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `eco honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `eco honcho sessions` | List known directory-to-session-name mappings |
| `eco honcho map <name>` | Map current working directory to a Honcho session name |
| `eco honcho identity` | Seed AI peer identity or show both peer representations |
| `eco honcho sync` | Create host blocks for all ECO profiles that don't have one yet |
| `eco honcho migrate` | Step-by-step migration guide from OpenClaw native memory to ECO + Honcho |
| `eco memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `eco memory status` | Show active memory provider and config |
| `eco memory off` | Disable external memory provider |
