...
version: 0.1.0
author: Bihruze (Asahi0x), ECO Agent
license: MIT
...
import upstream classes into ECO itself.
...
```bash
mkdir -p ~/.eco/cache/darwinian-evolver && cd ~/.eco/cache/darwinian-evolver
...
```bash
cd ~/.eco/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.eco/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.eco/skills/research/darwinian-evolver
DE_DIR=~/.eco/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside ECO core.
   Custom driver scripts under `~/.eco/skills/...` are user-side and fine.
