...
version: 1.0.0
author: arceus77-7, enhanced by ECO Agent
license: MIT
...
- `tmux` available for stable authenticated sessions during ECO terminal calls (desktop app flow only)
...
### Service Account (recommended for ECO)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.eco/.env` (the skill will prompt for this on first load).
...
## ECO Execution Pattern (desktop app flow)

ECO terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/eco-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/eco-op.sock"
