...
Usage (via cron with --no-agent):
    ...
    eco cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$ECO_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "ECO-Watcher/1.0"})
