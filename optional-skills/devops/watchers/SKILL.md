...
Watch a GitHub repo (set `GITHUB_TOKEN` in `~/.eco/.env` to avoid the 60 req/hr anonymous rate limit):

```bash
python $HERMES_HOME/skills/devops/watchers/scripts/watch_github.py \
  --name eco-issues --repo NousResearch/hermes-agent-agent --scope issues
```
