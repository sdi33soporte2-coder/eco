...
This module provides the glue:
    - ``ECOTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "ECO Agent"
"""
...
from eco_constants import secure_parent_dir
...
_USER_SKIPPED_SENTINEL = "__eco_user_skipped__"
...
def _get_token_dir() -> Path:
    ...
    Uses ECO_HOME so each profile gets its own OAuth tokens.
    Layout: ``ECO_HOME/mcp-tokens/``
    """
    try:
        from eco_constants import get_eco_home
        base = Path(get_eco_home())
    except ImportError:
        base = Path(os.environ.get("ECO_HOME", str(Path.home() / ".eco")))
...
# ---------------------------------------------------------------------------
# ECOTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class ECOTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        ECO_HOME/mcp-tokens/<server_name>.json         -- tokens
        ECO_HOME/mcp-tokens/<server_name>.client.json   -- client info
        ECO_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # ECO records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to ECO.</p></body></html>"
            ) if code else (
...
async def _redirect_handler(authorization_url: str) -> None:
    ...
    if _oauth_port and (os.getenv("SSH_CLIENT") or os.getenv("SSH_TTY")):
        print(
            ...
            f"\n"
            f"  See: https://eco.nousresearch.com/docs/guides/oauth-over-ssh\n",
            file=sys.stderr,
        )
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `eco mcp login <server>` later to "
...
def remove_oauth_tokens(server_name: str) -> None:
    ...
    storage = ECOTokenStorage(server_name)
    storage.remove()
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "ECO Agent")
    scope = cfg.get("scope")
