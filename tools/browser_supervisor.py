...
One ``CDPSupervisor`` runs per ECO ``task_id`` that has a reachable CDP
...
DIALOG_BRIDGE_HOST = "eco-dialog-bridge.invalid"
...
(() => {
  if (window.__ecoDialogBridgeInstalled) return;
  window.__ecoDialogBridgeInstalled = true;
  const ENDPOINT = "http://eco-dialog-bridge.invalid/";
