#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the ECO Skills Hub.
...
Used by eco_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class OptionalSkillSource(SkillSource):
    ...
    ~/.eco/skills/ during setup.  They are discoverable via the Skills Hub
...
# ---------------------------------------------------------------------------
# ECO centralized index source
# ---------------------------------------------------------------------------

ECO_INDEX_URL = "https://eco.nousresearch.com/docs/api/skills-index.json"
ECO_INDEX_CACHE_FILE = INDEX_CACHE_DIR / "eco-index.json"
ECO_INDEX_TTL = 6 * 3600  # 6 hours


def _load_eco_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for ECO_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    if ECO_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - ECO_INDEX_CACHE_FILE.stat().st_mtime
            if age < ECO_INDEX_TTL:
                return json.loads(ECO_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(ECO_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug("ECO index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("ECO index fetch failed: %s", e)
        return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        ECO_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        ECO_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
...
class ECOIndexSource(SkillSource):
    """Skill source backed by the centralized ECO Skills Index.
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "eco-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "eco-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        ECOIndexSource(auth=auth), # Centralized index (search + resolved install paths)
