...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``eco tools`` repaint, tool-registration scans,
    ...
    2. ``~/.eco/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
    ...
    try:
        ...
        auth_path = get_eco_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.eco/.env`` first, then ``os.environ``.

    Wraps :func:`eco_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from eco_cli.config import get_env_value as _eco_get_env_value

        value = _eco_get_env_value(name)
...
def eco_xai_user_agent() -> str:
    """Return a stable ECO-specific User-Agent for xAI HTTP calls."""
    try:
        from eco_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"ECO-Agent/{__version__}"


def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    """Resolve bearer credentials for direct xAI HTTP endpoints.

    Prefers ECO-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``eco_cli.config.get_env_value`` so keys
    stored in ``~/.eco/.env`` (the standard ECO location) are honored —
