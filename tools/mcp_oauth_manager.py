...
`mcp_tool.py`, and `eco_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# ECOMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_eco_provider_class() -> Optional[type]:
    ...
    class ECOMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(self, *args: Any, server_name: str = "", **kwargs: Any):
            ...
            self._eco_server_name = server_name
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        logger.debug(
                            ...
                            "token_endpoint=%s",
                            self._eco_server_name, asm.token_endpoint,
                        )
                        break
    ...
    return ECOMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_ECO_PROVIDER_CLS: Optional[type] = _make_eco_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`ECOMCPOAuthProvider` directly using the helpers
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``eco mcp remove <name>`` and (indirectly) by
        ``eco mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
