...
-----------------------------------------------------------------------------
    ...
    ~/.eco/checkpoints/
        store/                          — single bare-ish git repo
            ...
            refs/eco/<hash16>        — per-project branch tip
...
CHECKPOINT_BASE = get_eco_home() / "checkpoints"

# Single shared store directory under CHECKPOINT_BASE.
_STORE_DIRNAME = "store"
_REFS_PREFIX = "refs/eco"
...
DEFAULT_EXCLUDES = [
    ...
    ".hg/",
    ".svn/",
    # Worktrees (ECO convention — don't recursively snapshot siblings)
    ".worktrees/",
...
_GIT_TIMEOUT: int = max(10, min(60, int(os.getenv("ECO_CHECKPOINT_TIMEOUT", "30"))))
...
def _git_env(
    ...
    The shared store is internal ECO infrastructure — it must NOT inherit
...
def _migrate_legacy_store(base: Path) -> Optional[Path]:
    ...
    cleared with ``eco checkpoints clear-legacy``.
    ...
    if legacy_root is not None:
        logger.info(
            ...
            "Clear with `eco checkpoints clear-legacy` when safe.",
            legacy_root,
        )
    return legacy_root
...
def _init_store(store: Path, working_dir: str) -> Optional[str]:
    ...
    cfg_wd = str(base)
    _run_git(["config", "user.email", "eco@local"], store, cfg_wd)
    _run_git(["config", "user.name", "ECO Checkpoint"], store, cfg_wd)
...
# look for ``HEAD``/``info/exclude``/``ECO_WORKDIR``.  In v2 we also write
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    # Compat marker for tests that look at ECO_WORKDIR
    # (write in addition to the JSON metadata).
    try:
        (shadow_repo / "ECO_WORKDIR").write_text(
...
# ---------------------------------------------------------------------------
# Public helpers for `eco checkpoints` CLI
# ---------------------------------------------------------------------------
