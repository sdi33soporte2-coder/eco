...
Verifies:
  - Tools are gated on ECO_KANBAN_TASK: a normal chat session sees
...
def test_kanban_tools_hidden_without_env_var(monkeypatch, tmp_path):
    """Normal `eco chat` sessions (no ECO_KANBAN_TASK) must have
    zero kanban_* tools in their schema."""
    monkeypatch.delenv("ECO_KANBAN_TASK", raising=False)
    home = tmp_path / ".eco"
    home.mkdir()
    ...
    schema = registry.get_definitions(set(resolve_toolset("eco-cli")), quiet=True)
...
def test_kanban_tools_visible_with_env_var(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("ECO_KANBAN_TASK", "t_fake")
    home = tmp_path / ".eco"
    home.mkdir()
    monkeypatch.setenv("ECO_HOME", str(home))
...
def test_worker_with_kanban_toolset_still_hides_board_routing(monkeypatch, tmp_path):
    ...
    in its config, the ECO_KANBAN_TASK env var means it's a focused
...
def worker_env(monkeypatch, tmp_path):
    """Simulate being a worker: ECO_HOME isolated, ECO_KANBAN_TASK set
    after we've created the task."""
    home = tmp_path / ".eco"
    home.mkdir()
    monkeypatch.setenv("ECO_HOME", str(home))
    monkeypatch.setenv("ECO_PROFILE", "test-worker")
    ...
    try:
        ...
        conn.close()
    monkeypatch.setenv("ECO_KANBAN_TASK", tid)
    return tid
...
def test_complete_metadata_round_trips_through_show(worker_env):
    ...
    handoff = {
        "changed_files": ["eco_cli/kanban.py"],
...
def test_complete_stamps_worker_session_id_from_env(monkeypatch, worker_env):
    ...
    monkeypatch.setenv("ECO_SESSION_ID", "session-trusted")
...
def test_comment_happy_path(worker_env):
    ...
    try:
        ...
        # Author defaults to ECO_PROFILE env we set in the fixture
...
def test_comment_ignores_caller_supplied_author(worker_env):
    ...
    derived from ``ECO_PROFILE`` so a worker can't forge a comment
    under an authoritative-looking name like ``eco-system`` and
    ...
    from tools import kanban_tools as kt
    out = kt._handle_comment({
        "task_id": worker_env, "body": "hi", "author": "eco-system",
    })
    ...
    try:
        comments = kb.list_comments(conn, worker_env)
        # Author comes from ECO_PROFILE in the fixture, not the
        # caller-supplied "eco-system" override.
        assert comments[0].author == "test-worker"
    finally:
        conn.close()
...
def test_create_session_id_arg_overrides_env(monkeypatch, worker_env):
    ...
    monkeypatch.setenv("ECO_SESSION_ID", "from-env")
    from tools import kanban_tools as kt
    from eco_cli import kanban_db as kb
    out = kt._handle_create({
...
def test_kanban_guidance_not_in_normal_prompt(monkeypatch, tmp_path):
    """A normal chat session (no ECO_KANBAN_TASK) must NOT have
...
def test_kanban_guidance_in_worker_prompt(monkeypatch, tmp_path):
    """A worker session (ECO_KANBAN_TASK set) MUST have the full
...
# ---------------------------------------------------------------------------
#
# A worker process has ECO_KANBAN_TASK set to its own task id. The
...
# are unrestricted.
#
# Orchestrator profiles (no ECO_KANBAN_TASK in env) are intentionally
...
def test_worker_can_comment_on_foreign_task(worker_env):
    ...
    # ECO_PROFILE — never to a caller-controlled string.
    conn = kb.connect()
    try:
...
def test_worker_complete_rejects_stale_run_id(worker_env, monkeypatch):
    ...
    from tools import kanban_tools as kt
    monkeypatch.setenv("ECO_KANBAN_RUN_ID", str(run1.id))
    ...
    monkeypatch.setenv("ECO_KANBAN_RUN_ID", str(run2.id))
...
def test_orchestrator_complete_any_task_allowed(monkeypatch, tmp_path):
    """Orchestrator profiles (no ECO_KANBAN_TASK) can still complete
...
# ---------------------------------------------------------------------------
#
# The dispatcher pins the active board via ECO_KANBAN_BOARD env var,
...
# restarting ECO. These tests pin that ``board=<slug>`` argument
...
def multi_board_env(monkeypatch, tmp_path):
    """Isolated ECO home with two distinct kanban boards seeded.

    Returns ``("default", "alt")`` slugs. The default board has one
    pre-existing task ``seed_default``; ``alt`` has ``seed_alt``. No
    ECO_KANBAN_TASK is pinned (orchestrator context) — workers test
    the env-task case via the existing ``worker_env`` fixture.
    """
    home = tmp_path / ".eco"
    home.mkdir()
    monkeypatch.setenv("ECO_HOME", str(home))
    # Make sure neither ECO_KANBAN_DB nor ECO_KANBAN_BOARD pin a
    ...
    monkeypatch.delenv("ECO_KANBAN_TASK", raising=False)
    monkeypatch.setenv("ECO_PROFILE", "test-orchestrator")
    from pathlib import Path as _Path
    monkeypatch.setattr(_Path, "home", lambda: tmp_path)
...
def test_board_param_routes_heartbeat_to_alt_board(monkeypatch, tmp_path):
    ...
    use the worker-env style fixture inline (pinning ECO_KANBAN_TASK
    to a task that exists in the alt board)."""
    home = tmp_path / ".eco"
    home.mkdir()
    monkeypatch.setenv("ECO_HOME", str(home))
    monkeypatch.setenv("ECO_PROFILE", "alt-worker")
    monkeypatch.delenv("ECO_KANBAN_DB", raising=False)
    monkeypatch.delenv("ECO_KANBAN_BOARD", raising=False)
    from pathlib import Path as _Path
    monkeypatch.setattr(_Path, "home", lambda: tmp_path)
