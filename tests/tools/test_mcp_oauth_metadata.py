...
def _manager_provider_with_context(storage: HermesTokenStorage, **context_attrs):
    ...
    provider = _ECO_PROVIDER_CLS.__new__(_ECO_PROVIDER_CLS)
    provider._eco_server_name = context_attrs.get("server_name", "srv")
    context = MagicMock()
...
class TestManagerOAuthProviderMetadata:
    def test_initialize_restores_metadata_from_disk(self, tmp_path, monkeypatch):
        ...
        with patch.object(
            _ECO_PROVIDER_CLS.__bases__[0], "_initialize", new=AsyncMock()
        ):
