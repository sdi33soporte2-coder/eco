...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad ECO_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: ECO_GATEWAY_SESSION set, a bound session
    key, and isolated gateway queues/callbacks. Yields the session_key."""
    monkeypatch.setenv("ECO_GATEWAY_SESSION", "1")
...
def test_env_scrub_eco_allowlist_and_secret_blocks():
    ...
    env = {
        ...
        "ECO_HOME": "/h", "ECO_PROFILE": "p",
        "ECO_CONFIG": "/c.yaml", "ECO_ENV": "/e",
        # other ECO_* → dropped (broad prefix removed)
        "ECO_BASE_URL": "https://x", "ECO_INTERACTIVE": "1",
        "ECO_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in ("ECO_HOME", "ECO_PROFILE", "ECO_CONFIG", "ECO_ENV", "PATH"):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "ECO_BASE_URL", "ECO_INTERACTIVE", "ECO_KANBAN_DB",
...
def test_env_scrub_logs_dropped_eco_vars(caplog):
    """Dropping a non-allowlisted, non-secret ECO_* var must be diagnosable:
    ...
    leave users guessing why a sandbox script sees an unset ECO_* var."""
    import logging

    from tools.code_execution_tool import _scrub_child_env

    env = {
        "ECO_HOME": "/h",          # allowlisted → kept, not logged
        "ECO_BASE_URL": "https://x",   # dropped → logged
        "ECO_KANBAN_DB": "postgres://u:p@h/db",  # dropped → logged
        "ECO_API_KEY": "sk",       # secret → dropped silently (not logged)
        "PATH": "/usr/bin",           # safe prefix → kept
    }
    with caplog.at_level(logging.DEBUG, logger="tools.code_execution_tool"):
        out = _scrub_child_env(env, is_passthrough=lambda _: False, is_windows=False)

    assert "ECO_HOME" in out and "PATH" in out
    assert "ECO_BASE_URL" not in out and "ECO_KANBAN_DB" not in out

    msgs = "\n".join(r.getMessage() for r in caplog.records)
    assert "ECO_BASE_URL" in msgs and "ECO_KANBAN_DB" in msgs
    assert "env_passthrough" in msgs
    # Secret vars are dropped but must NOT be named in the diagnostic log.
    assert "ECO_API_KEY" not in msgs


def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped ECO_* vars."""
    import logging
