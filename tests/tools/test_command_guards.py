...
def _clean_state():
    ...
    saved = {}
    for k in ("ECO_INTERACTIVE", "ECO_GATEWAY_SESSION", "ECO_EXEC_ASK", "ECO_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["ECO_INTERACTIVE"] = "1"
        session_key = os.getenv("ECO_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No ECO_INTERACTIVE or ECO_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
