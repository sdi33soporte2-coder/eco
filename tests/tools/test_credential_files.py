...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        eco_home.mkdir()
        (eco_home / "token.json").write_text("{}")
        ...
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(eco_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.eco/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        eco_home = tmp_path / ".eco"
        eco_home.mkdir()
        (eco_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        eco_home = tmp_path / ".eco"
        eco_home.mkdir()
        (eco_home / "secret.key").write_text("key")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.eco/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        eco_home = tmp_path / ".eco"
        (eco_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"ECO_HOME": str(eco_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.eco")

        assert mounts[0]["container_path"] == "/home/user/.eco/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.eco/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.eco/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape ECO_HOME."""
        monkeypatch.setenv("ECO_HOME", str(tmp_path / ".eco"))
        (tmp_path / ".eco").mkdir()

        # Create a sensitive file one level above eco_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside eco_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside ECO_HOME must still be registered."""
        eco_home = tmp_path / ".eco"
        eco_home.mkdir()
        monkeypatch.setenv("ECO_HOME", str(eco_home))
        (eco_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_eco_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of ECO_HOME must be allowed."""
        eco_home = tmp_path / ".eco"
        eco_home.mkdir()
        subdir = eco_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside ECO_HOME pointing outside must be rejected."""
        eco_home = tmp_path / ".eco"
        eco_home.mkdir()
        monkeypatch.setenv("ECO_HOME", str(eco_home))

        # Create a sensitive file outside eco_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside eco_home pointing outside
        symlink = eco_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes ECO_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, eco_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape ECO_HOME."""
        eco_home = tmp_path / ".eco"
        eco_home.mkdir()
        monkeypatch.setenv("ECO_HOME", str(eco_home))
        ...
        sensitive.write_text("{}")
        self._write_config(eco_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(eco_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside ECO_HOME via config must still mount."""
        eco_home = tmp_path / ".eco"
        eco_home.mkdir()
        monkeypatch.setenv("ECO_HOME", str(eco_home))

        (eco_home / "oauth.json").write_text("{}")
        self._write_config(eco_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        eco_home = tmp_path / ".eco"
        eco_home.mkdir()
        (eco_home / "cache" / "documents").mkdir(parents=True)
        (eco_home / "cache" / "audio").mkdir(parents=True)
        monkeypatch.setenv("ECO_HOME", str(eco_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.eco/cache/documents" in paths
        assert "/root/.eco/cache/audio" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.eco/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        eco_home = tmp_path / ".eco"
        eco_home.mkdir()
        # Use legacy dir name — get_eco_dir prefers old if it exists
        (eco_home / "document_cache").mkdir()
        (eco_home / "image_cache").mkdir()
        monkeypatch.setenv("ECO_HOME", str(eco_home))

        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(eco_home / "document_cache") in host_paths
        assert str(eco_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.eco/cache/documents" in container_paths
        assert "/root/.eco/cache/images" in container_paths
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        eco_home = tmp_path / ".eco"
        doc_dir = eco_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        eco_home = tmp_path / ".eco"
        ss_dir = eco_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.eco/cache/screenshots/session_abc/screen1.png"
