...
Verifies that ECO-managed provider, tool, and gateway env vars are
stripped from subprocess environments so external CLIs are not silently
misrouted or handed ECO secrets.

See: https://github.com/NousResearch/hermes-agent-agent-agent/issues/1002
See: https://github.com/NousResearch/hermes-agent-agent-agent/issues/1264
"""
...
class TestProviderEnvBlocklist:
    """Provider env vars loaded from ~/.eco/.env must not leak."""
    ...
    def test_bedrock_bearer_token_is_stripped(self):
        """The Bedrock-specific bearer token is a ECO inference secret
        ...
        models`` run inside a ECO terminal enumerated the entire Bedrock
...
class TestBlocklistCoverage:
    ...
    def test_general_aws_chain_not_in_blocklist(self):
        ...
        operator shell (SECURITY.md §3.2), not to ECO, and blocklisting
