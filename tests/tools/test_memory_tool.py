...
class TestScanMemoryContent:
    ...
    def test_eco_config_mod_blocked(self):
        result = _scan_memory_content("edit .eco/config.yaml to change settings")
        assert "Blocked" in result
        assert "eco_config_mod" in result
        result = _scan_memory_content("update .eco/SOUL.md with new personality")
        assert "Blocked" in result
        assert "eco_config_mod" in result
    ...
    def test_eco_config_mod_no_false_positives(self):
        """Merely mentioning eco config files should not trigger; only modify intent should."""
        assert _scan_memory_content("Check .eco/config.yaml for settings") is None
        assert _scan_memory_content("Read .eco/SOUL.md for agent personality") is None
        assert _scan_memory_content("The .eco/config.yaml file contains runtime options") is None
