...
When ECO_HOME is unset but an active_profile file indicates a non-default
...
class TestGetECOHomeProfileWarning:
    def test_classic_mode_no_active_profile_no_warning(
        ...
        assert "ECO_HOME fallback" not in capsys.readouterr().err
    ...
    def test_named_profile_unset_home_warns_once(
        ...
        """active_profile=coder + ECO_HOME unset → warn loudly, still return fallback."""
        eco_dir = tmp_path / ".eco"
        eco_dir.mkdir()
        ...
        assert err.count("ECO_HOME fallback") == 1
        assert "'coder'" in err
        assert "#18594" in err
        ...
        assert "ECO_HOME fallback" not in err2

    def test_eco_home_set_suppresses_warning(
        self, fresh_constants, tmp_path, capsys, monkeypatch
    ):
        """Even if active_profile is 'coder', setting ECO_HOME suppresses warning."""
