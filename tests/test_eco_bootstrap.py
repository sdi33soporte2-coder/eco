"""Tests for eco_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every ECO entry point
(eco, eco-agent, eco-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every ECO entry point imports eco_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported eco_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("eco_bootstrap", None)
    import eco_bootstrap  # noqa: WPS433
    return eco_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the ECO CLI
...
class TestEntryPointsImportBootstrap:
    """Every ECO entry point must import eco_bootstrap as its
    ...
    # Entry points that invoke ECO as a process.  Each one must
    # import eco_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "eco_cli/main.py",   # eco CLI (console_script)
        "run_agent.py",          # eco-agent (console_script)
        "acp_adapter/entry.py",  # eco-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import eco_bootstrap' and that
        ...
        half-finished ``eco update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``eco_bootstrap`` as a top-level module) leaves eco
        ...
        """
        # Resolve relative to the eco-agent repo root.  Tests live
        # at tests/test_eco_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # eco start even when eco_bootstrap hasn't been
        ...
        assert first_import_name == "eco_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'eco_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import eco_bootstrap' line to be the first import."
        )
