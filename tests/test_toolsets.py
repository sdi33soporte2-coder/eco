...
class TestToolsetConsistency:
    ...
    def test_eco_platforms_share_core_tools(self):
        """All eco-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on eco-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["eco-cli", "eco-telegram", "eco-discord", "eco-whatsapp", "eco-slack", "eco-signal", "eco-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_eco_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("eco-whatsapp")

    def test_eco_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("eco-api-server")
