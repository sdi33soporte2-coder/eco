...
"""
    ...
    assert "Tyler Williams > Active Projects: ECO Agent" in entries
...
def test_cron_store_is_archived_without_config_cron_section(tmp_path: Path):
    ...
    assert "Run `eco cron` to recreate scheduled tasks" in notes_text
...
def test_skill_installs_cleanly_under_skills_guard():
    ...
    # eco_config_mod  — print statements in the post-migration summary that
    #                      tell the user to *review* ~/.eco/config.yaml;
    ...
    KNOWN_FALSE_POSITIVES = {"agent_config_mod", "python_os_environ", "eco_config_mod"}
...
def test_rebrand_text_replaces_openclaw_variants():
    mod = load_module()
    # Mixed-case / capitalized matches → capital-H ``ECO``.
    assert mod.rebrand_text("OpenClaw prefers Python 3.11") == "ECO prefers Python 3.11"
    assert mod.rebrand_text("I told Open Claw to use dark mode") == "I told ECO to use dark mode"
    assert mod.rebrand_text("Open-Claw config is great") == "ECO config is great"
    assert mod.rebrand_text("OPENCLAW uses tools well") == "ECO uses tools well"
    # All-lowercase matches → lowercase ``eco``; this preserves the
    # real filesystem path ``~/.eco`` (ECO home) when rebranding
    # memory entries that reference ``~/.openclaw`` or ``openclaw`` prose.
    assert mod.rebrand_text("openclaw should always respond concisely") == "eco should always respond concisely"


def test_rebrand_text_replaces_legacy_bot_names():
    mod = load_module()
    # Same case-preservation rule as above.
    assert mod.rebrand_text("ClawdBot remembers my timezone") == "ECO remembers my timezone"
    assert mod.rebrand_text("clawdbot prefers tabs") == "eco prefers tabs"
    assert mod.rebrand_text("MoltBot was configured for Spanish") == "ECO was configured for Spanish"
    assert mod.rebrand_text("moltbot uses Python") == "eco uses Python"
...
def test_rebrand_text_handles_multiple_replacements():
    ...
    assert mod.rebrand_text(text) == "ECO said to ask ECO about ECO settings"


def test_rebrand_text_preserves_filesystem_path_casing():
    """Lowercase matches — especially ``.openclaw`` filesystem paths — must
    rewrite to lowercase ``.eco`` (the real ECO home), not the broken
    ``.ECO``.
    ...
    rewritten to ``~/.ECO/config.yaml`` — a path that doesn't exist —
    and the agent kept trying to read it.
    """
    mod = load_module()
    assert mod.rebrand_text("config is at ~/.openclaw/config.yaml") == \
        "config is at ~/.eco/config.yaml"
    assert mod.rebrand_text("use .openclaw directory") == "use .eco directory"
    assert mod.rebrand_text("Path.home() / '.openclaw'") == "Path.home() / '.eco'"
