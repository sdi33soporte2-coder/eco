...
normal tests assume away:
  ...
  - Filesystem: ECO_HOME with spaces / unicode / symlinks.
...
def scenario(name):
    """Decorator: run `fn` in its own ECO_HOME, collect failures.
    ...
    def wrap(fn):
        def run():
            home = tempfile.mkdtemp(prefix=f"eco_atyp_{name}_")
            os.environ["ECO_HOME"] = home
            os.environ["HOME"] = home
            for m in list(sys.modules.keys()):
                if m.startswith(("eco_cli", "plugins", "gateway")):
                    del sys.modules[m]
            sys.path.insert(0, str(WT))
            from eco_cli import kanban_db as kb  # noqa: F401
            print(f"\n═══ {name} ═══")
            try:
                fn(home, kb)
                print(f"  ✔ {name}")
...
def _(home, kb):
    ...
    env = {**os.environ, "PYTHONPATH": str(WT), "ECO_HOME": home, "HOME": home}
    bad_metas = [
        "not-json",
    ...
    for bad in bad_metas:
        r = subprocess.run(
            [sys.executable, "-m", "eco_cli.main", "kanban",
...
def _(home, kb):
    ...
    try:
        ...
        # allow escape.
        try:
            from eco_cli.kanban_db import resolve_workspace
            ...
            home_abs = str(Path(os.environ["ECO_HOME"]).resolve())
            if not resolved_abs.startswith(home_abs) and resolved_abs.startswith("/tmp"):
                ...
                print(f"  ⚠ workspace resolved OUTSIDE eco_home: {resolved}")
...
@scenario("eco_home_with_spaces")
def _(home, kb):
    """ECO_HOME at a path with spaces — should work but catches
    ...
    weird = tempfile.mkdtemp(prefix="eco with spaces ")
    os.environ["ECO_HOME"] = weird
    ...
    try:
        ...
        print(f"  ECO_HOME with spaces: OK at {weird}")
    finally:
        conn.close()
        shutil.rmtree(weird, ignore_errors=True)


@scenario("eco_home_with_unicode")
def _(home, kb):
    """ECO_HOME with non-ASCII chars."""
    # Pre-create directly since tempfile doesn't love unicode prefixes
    weird = f"/tmp/eco_héllo_émöji_{os.getpid()}"
    ...
    try:
        ...
        print(f"  ECO_HOME with unicode path: OK at {weird}")
    finally:
        conn.close()
        shutil.rmtree(weird, ignore_errors=True)


@scenario("eco_home_via_symlink")
def _(home, kb):
    """ECO_HOME is a symlink to the real dir. _INITIALIZED_PATHS
    uses Path.resolve() — two different symlink names pointing at the
    same dir should NOT double-init."""
    real = tempfile.mkdtemp(prefix="eco_real_")
    link1 = real + "_link1"
    link2 = real + "_link2"
    os.symlink(real, link1)
    os.symlink(real, link2)
    try:
        os.environ["ECO_HOME"] = link1
        ...
        conn2.close()
        print("  symlinks to same ECO_HOME share DB correctly")
    finally:
...
def _(home, kb):
    ...
    try:
        from eco_cli import web_server as ws  # noqa
    except Exception:
        pass
