"""Tests for ACP Registry metadata shipped with ECO."""
...
def test_agent_json_matches_official_registry_required_fields():
    ...
    assert data["id"] == "eco-agent"
    assert re.fullmatch(r"[a-z][a-z0-9-]*", data["id"])
    assert data["name"] == "ECO Agent"
    assert data["description"]
    assert data["repository"] == "https://github.com/NousResearch/hermes-agent-agent-agent"
    assert data["website"].startswith("https://eco-agent.nousresearch.com/")
...
def test_agent_json_uses_uvx_distribution_without_local_command_fields():
    ...
    assert uvx["package"] == f"eco-agent[acp]=={data['version']}"
    assert uvx["args"] == ["eco-acp"]
...
def test_agent_json_pins_uvx_package_to_pyproject_version():
    ...
    assert _manifest()["distribution"]["uvx"]["package"] == (
        f"eco-agent[acp]=={_pyproject_version()}"
    )
