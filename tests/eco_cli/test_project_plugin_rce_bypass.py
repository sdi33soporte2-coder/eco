...
Execution via the ``ECO_ENABLE_PROJECT_PLUGINS`` bypass in the web
...
1. ``eco_cli.web_server._discover_dashboard_plugins`` opted into
   ...
   ``os.environ.get("ECO_ENABLE_PROJECT_PLUGINS")`` — truthy for
...
# ---------------------------------------------------------------------------
# Layer 1 — ECO_ENABLE_PROJECT_PLUGINS env gate uses truthy semantics.
# ---------------------------------------------------------------------------
...
class TestProjectPluginsEnvGate:
    ...
    def project_plugin(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ECO_HOME", str(tmp_path / "home"))
        (tmp_path / "home").mkdir()
    ...
    def test_falsy_values_keep_project_plugins_disabled(
        ...
        if value == "":
            monkeypatch.delenv("ECO_ENABLE_PROJECT_PLUGINS", raising=False)
        else:
            monkeypatch.setenv("ECO_ENABLE_PROJECT_PLUGINS", value)

        plugins = web_server._get_dashboard_plugins(force_rescan=True)
        names = {p["name"] for p in plugins}
        assert "evil" not in names, (
            f"ECO_ENABLE_PROJECT_PLUGINS={value!r} must NOT enable the "
...
class TestEndToEndPocBlocked:
    ...
    ``ECO_ENABLE_PROJECT_PLUGINS=0`` (so the operator believed the
    ...
    def test_full_chain_blocked(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ECO_ENABLE_PROJECT_PLUGINS", "0")
