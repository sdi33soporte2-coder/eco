"""Tests for eco_cli.model_catalog — remote manifest fetch + cache + fallback."""
...
def isolated_home(tmp_path, monkeypatch):
    """Isolate ECO_HOME + reset any module-level catalog cache per test."""
    home = tmp_path / ".eco"
    home.mkdir()
    ...
    import importlib
    from eco_cli import model_catalog
...
class TestValidation:
    def test_accepts_well_formed_manifest(self, isolated_home):
        from eco_cli.model_catalog import _validate_manifest
...
class TestFallbackChain:
    ...
    PRIMARY = "https://eco-agent.nousresearch.com/docs/api/model-catalog.json"
    FALLBACK = (
        "https://raw.githubusercontent.com/NousResearch/hermes-agent-agent-agent"
...
class TestIntegrationWithModelsModule:
    """Exercise the fallback paths via the real callers in eco_cli.models."""

    def test_curated_nous_ids_falls_back_to_hardcoded_on_empty_catalog(
        self, isolated_home
    ):
        from eco_cli import model_catalog
        from eco_cli.models import get_curated_nous_model_ids, _PROVIDER_MODELS
    ...
    def test_curated_nous_ids_prefers_manifest(self, isolated_home):
        from eco_cli import model_catalog
        from eco_cli.models import get_curated_nous_model_ids
    ...
    def test_picker_nous_row_uses_manifest(self, tmp_path, monkeypatch):
        ...
        the slash-command picker until the next ECO release.
        """
        ...
        # ``ECO_HOME / auth.json`` then resolves to the same path the
        # seat-belt thinks is the "real" user store. Use the autouse
        # ``_hermetic_environment`` ECO_HOME directly instead.
        import importlib
        from eco_cli import model_catalog
        importlib.reload(model_catalog)
        try:
            from eco_cli.model_switch import list_picker_providers

            active_home = Path(os.environ["ECO_HOME"])
