"""Comprehensive tests for eco_cli.profiles module.
...
# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and ECO_HOME for profile tests
# ---------------------------------------------------------------------------
...
def profile_env(tmp_path, monkeypatch):
    ...
    * Path.home() -> tmp_path  (so _get_profiles_root() = tmp_path/.eco/profiles)
    * ECO_HOME  -> tmp_path/.eco  (so get_eco_home() agrees)
    * Creates the bare-minimum ~/.eco directory.
    """
...
class TestValidateProfileName:
    ...
    @pytest.mark.parametrize("name", ["eco", "test", "tmp", "root", "sudo"])
    def test_reserved_names_rejected(self, name):
        """Reserved names collide with the ECO install itself or with
...
class TestGetProfileDir:
    ...
    def test_default_returns_eco_home(self, profile_env):
    ...
    def test_named_profile_returns_profiles_subdir(self, profile_env):
        ...
        assert result == tmp_path / ".eco" / "profiles" / "coder"

    def test_named_profile_matching_is_case_insensitive(self, profile_env):
        tmp_path = profile_env
        assert get_profile_dir("Coder") == tmp_path / ".eco" / "profiles" / "coder"
...
class TestCreateProfile:
    ...
    def test_clone_all_excludes_sibling_profiles_tree(self, profile_env):
        """--clone-all from default ~/.eco must not copy profiles/* (nested explosion)."""
        tmp_path = profile_env
        default_home = tmp_path / ".eco"
    ...
    def test_clone_all_excludes_default_infrastructure(self, profile_env):
        """--clone-all from default profile excludes eco-agent, .worktrees,
        ...
        (default_home / "eco-agent" / ".git").mkdir(parents=True)
        (default_home / "eco-agent" / "venv" / "bin").mkdir(parents=True)
        (default_home / "eco-agent" / "README.md").write_text("repo")
        ...
        assert not (profile_dir / "eco-agent").exists()
...
class TestNoSkillsOptOut:
    """Tests for `eco profile create --no-skills` and the opt-out marker."""
    ...
    def test_seed_profile_skills_respects_marker(self, profile_env):
        ...
        called directly (e.g. by `eco update`'s all-profile sync loop)."""
        profile_dir = create_profile("orchestrator", no_alias=True, no_skills=True)
...
class TestDeleteProfile:
    ...
    def test_removes_directory(self, profile_env):
        ...
        with patch("eco_cli.profiles._cleanup_gateway_service"):
...
class TestActiveProfile:
    ...
    def test_empty_file_returns_default(self, profile_env):
        tmp_path = profile_env
        active_path = tmp_path / ".eco" / "active_profile"
...
class TestGetActiveProfileName:
    ...
    def test_default_eco_home_returns_default(self, profile_env):
        # ECO_HOME points to tmp_path/.eco which is the default
    ...
    def test_custom_path_returns_default(self, profile_env, monkeypatch):
        """A custom ECO_HOME (Docker, etc.) IS the default root."""
        tmp_path = profile_env
        custom = tmp_path / "some" / "other" / "path"
        custom.mkdir(parents=True)
        monkeypatch.setenv("ECO_HOME", str(custom))
        # With Docker-aware roots, a custom ECO_HOME is the default —
...
class TestAliasCollision:
    ...
    def test_reserved_name_returns_message(self, profile_env):
        result = check_alias_collision("eco")
        assert result is not None
...
class TestRenameProfile:
    ...
    def test_renames_root_honcho_host_without_changing_ai_peer(self, profile_env):
        ...
        assert "eco.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["eco.heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["eco.heimdall"]["peerName"] == "user-peer"
    ...
    def test_pins_ai_peer_when_absent_on_honcho_host_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "eco.ssi_health": {"workspace": "eco", "enabled": True}
            }
        }))
        ...
        assert "eco.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["eco.heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["eco.heimdall"]["workspace"] == "eco"
    ...
    def test_does_not_overwrite_existing_honcho_host_on_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "eco.ssi_health": {"aiPeer": "ssi_health"},
                "eco.heimdall": {"aiPeer": "heimdall"},
            }
        }))
        ...
        cfg = json.loads(honcho_path.read_text())
        assert cfg["hosts"]["eco.ssi_health"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["eco.heimdall"]["aiPeer"] == "heimdall"
...
class TestExportImport:
    ...
    def test_export_default_excludes_infrastructure(self, profile_env, tmp_path):
        ...
        for d in ("eco-agent", ".worktrees", "profiles", "bin",
        ...
        for f in ("state.db", "gateway.pid", "gateway_state.json",
                  "processes.json", "errors.log", ".eco_history",
        ...
        excluded_prefixes = [
            "default/eco-agent", "default/.worktrees", "default/profiles",
        ...
        excluded_files = [
            ...
            "default/errors.log", "default/.eco_history",
...
# ===================================================================
# TestGetProfilesRoot / TestGetDefaultECOHome (internal helpers)
# ===================================================================

class TestInternalHelpers:
    """Tests for _get_profiles_root() and _get_default_eco_home()."""

    def test_profiles_root_under_home(self, profile_env):
        tmp_path = profile_env
        root = _get_profiles_root()
        assert root == tmp_path / ".eco" / "profiles"

    def test_default_eco_home(self, profile_env):
        tmp_path = profile_env
        home = _get_default_eco_home()
        assert home == tmp_path / ".eco"

    def test_profiles_root_docker_deployment(self, tmp_path, monkeypatch):
        """In Docker (ECO_HOME outside ~/.eco), profiles go under ECO_HOME."""
    ...
    def test_default_eco_home_docker(self, tmp_path, monkeypatch):
        """In Docker, _get_default_eco_home() returns ECO_HOME itself."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ECO_HOME", str(docker_home))
    ...
    def test_profiles_root_profile_mode(self, tmp_path, monkeypatch):
        """In profile mode (ECO_HOME under ~/.eco), profiles root is still ~/.eco/profiles."""
        native = tmp_path / ".eco"
    ...
    def test_active_profile_path_docker(self, tmp_path, monkeypatch):
        """In Docker, active_profile file lives under ECO_HOME."""
        from eco_cli.profiles import _get_active_profile_path
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ECO_HOME", str(docker_home))
    ...
    def test_create_profile_docker(self, tmp_path, monkeypatch):
        """Profile created in Docker lands under ECO_HOME/profiles/."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ECO_HOME", str(docker_home))
...
class TestEdgeCases:
    ...
    def test_create_profile_returns_correct_path(self, profile_env):
        ...
        expected = tmp_path / ".eco" / "profiles" / "mybot"
        assert result == expected
    ...
    def test_gateway_running_check_with_pid_file(self, profile_env):
        ...
        from eco_cli.profiles import _check_gateway_running
        tmp_path = profile_env
        default_home = tmp_path / ".eco"
