...
board into `ECO_KANBAN_BOARD` at boot so subprocess shell-outs (e.g.
...
def _isolate_kanban_board_env():
    """Snapshot `ECO_KANBAN_BOARD` and restore it after the test.
    ...
    """
    prev = os.environ.get("ECO_KANBAN_BOARD")
    ...
    try:
        ...
        if prev is None:
            ...
            os.environ["ECO_KANBAN_BOARD"] = prev
...
def test_pin_writes_resolved_board_when_env_unset(monkeypatch):
    ...
    assert main_mod.os.environ.get("ECO_KANBAN_BOARD") == "space"


def test_pin_does_not_overwrite_existing_env(monkeypatch):
    monkeypatch.setenv("ECO_KANBAN_BOARD", "preset")
    main_mod = importlib.import_module("eco_cli.main")
    ...
    assert main_mod.os.environ.get("ECO_KANBAN_BOARD") == "preset"
...
def test_pin_swallows_resolution_failures(monkeypatch):
    ...
    assert "ECO_KANBAN_BOARD" not in main_mod.os.environ
