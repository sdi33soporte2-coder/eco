...
When HERMES_HOME is set to the eco root (e.g. systemd hardcodes
HERMES_HOME=/root/.eco), _apply_profile_override must still read
...
def _run_apply_profile_override(
    ...
    if eco_home is not None:
        monkeypatch.setenv("HERMES_HOME", eco_home)
    else:
...
class TestApplyProfileOverrideECOHomeGuard:
    ...
    Verifies that HERMES_HOME pointing to the eco root does NOT suppress
    ...
    def test_eco_home_at_root_with_active_profile_is_redirected(
        ...
        """HERMES_HOME=/root/.eco + active_profile=coder must redirect
        HERMES_HOME to .../profiles/coder.

        Bug scenario from #22502: systemd sets HERMES_HOME to the eco root
