"""Tests for IRC gateway configuration via `eco setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh ECO install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new ECO install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#eco")
            monkeypatch.setenv("IRC_NICKNAME", "eco-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `eco setup gateway` experience with IRC present."""
