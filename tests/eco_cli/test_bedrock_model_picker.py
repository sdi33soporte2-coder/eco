...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (ECO_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via ECO_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in ECO_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from eco_cli.providers import ECO_OVERLAYS
        assert "bedrock" in ECO_OVERLAYS

    def test_bedrock_overlay_transport(self):
        from eco_cli.providers import ECO_OVERLAYS
        assert ECO_OVERLAYS["bedrock"].transport == "bedrock_converse"

    def test_bedrock_overlay_auth_type(self):
        from eco_cli.providers import ECO_OVERLAYS
        assert ECO_OVERLAYS["bedrock"].auth_type == "aws_sdk"
