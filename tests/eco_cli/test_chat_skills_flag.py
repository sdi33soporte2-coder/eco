...
def test_top_level_skills_flag_defaults_to_chat(monkeypatch):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["eco", "-s", "eco-agent-dev,github-auth"],
    )

    main_mod.main()

    assert captured == {
        "skills": ["eco-agent-dev,github-auth"],
        "command": None,
    }
...
def test_chat_subcommand_accepts_skills_flag(monkeypatch):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["eco", "chat", "-s", "github-auth", "-q", "hello"],
    )
...
def test_chat_subcommand_accepts_image_flag(monkeypatch):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["eco", "chat", "-q", "hello", "--image", "~/storage/shared/Pictures/cat.png"],
    )
