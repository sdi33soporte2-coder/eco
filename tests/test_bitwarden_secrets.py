...
binary download are exercised manually by `eco secrets bitwarden
...
def test_install_bws_happy_path(eco_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(eco_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(eco_home, monkeypatch):
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under eco_home/cache/."""
    home = tmp_path / ".eco"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
