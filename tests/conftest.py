"""Shared fixtures for the eco-agent test suite.
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    #    fixture. Any code in the codebase reading ``~/.eco/*`` via
    #    ``Path.home() / ".eco"`` instead of ``get_eco_home()``
    #    is a bug to fix at the callsite.
    fake_eco_home = tmp_path / "eco_test"
    fake_eco_home.mkdir()
    (fake_eco_home / "sessions").mkdir()
    ...
    #    ~/.eco/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import eco_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_eco_home(_hermetic_environment):
...
@pytest.fixture()
def mock_config():
    """Return a minimal eco config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``eco-gateway`` process
...
# correlated with a ``tests/eco_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> eco-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart eco-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting eco/python patterns are also blocked.
    """
    ...
    def _is_blocked_systemctl(cmd) -> bool:
        ...
        if "systemctl" not in cmd_str:
            return False
        if not _matches_eco_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch eco-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m eco_cli.main".
                if (
                    "eco" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live eco-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting eco/python could hit the live gateway. "
