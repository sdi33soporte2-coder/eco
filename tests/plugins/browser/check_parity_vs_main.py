...
Run from the PR worktree:
    ...
    cd ~/.eco/eco-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.eco/eco-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.eco/eco-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a eco-agent checkout"
)
...
# Isolated ECO_HOME for the config write.
