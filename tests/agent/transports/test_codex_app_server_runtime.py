...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. ECO passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("ECO_HOME", "/users/alice/.eco/profiles/backend-worker")
        monkeypatch.setenv("ECO_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "ECO_KANBAN_DB",
            "/users/alice/.eco/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.eco/kanban/boards/smoke"]'
            in cmd
        )
