"""Tests for the cross-ECO-profile write guard in agent/file_safety.

The guard fires when a tool tries to write into another ECO profile's
...
Reference: May 2026 incident — a eco-security profile session
accidentally edited skills under both ~/.eco/profiles/eco-security/skills/
AND ~/.eco/skills/ (the default profile's skills), realizing only
...
# ---------------------------------------------------------------------------
# Helpers — set up a fake ECO root with two profiles, monkeypatch the
...
def fake_hermes(tmp_path, monkeypatch):
    ...
    sec_home = root / "profiles" / "eco-security"
    ...
    monkeypatch.setattr(eco_constants, "get_default_eco_root", lambda: root)
...
def _set_active_home(monkeypatch, eco_home: Path):
    """Point file_safety._eco_home_path at a specific profile dir."""
    import agent.file_safety as fs
    monkeypatch.setattr(fs, "_eco_home_path", lambda: eco_home)
...
class TestResolveActiveProfileName:
    ...
    def test_named_profile(self, fake_hermes, monkeypatch):
        ...
        from agent.file_safety import _resolve_active_profile_name
        assert _resolve_active_profile_name() == "eco-security"

    def test_falls_back_to_default_on_resolution_failure(self, fake_hermes, monkeypatch):
        """If ECO_HOME resolution raises, return 'default' rather than crashing the tool."""
        import agent.file_safety as fs

        def _boom():
            raise RuntimeError("simulated")

        monkeypatch.setattr(fs, "_eco_home_path", _boom)
...
class TestClassifyCrossProfileTarget:
    ...
    def test_security_writing_default_skill(self, fake_hermes, monkeypatch):
        ...
        assert result["active_profile"] == "eco-security"
    ...
    def test_default_writing_security_skill(self, fake_hermes, monkeypatch):
        ...
        assert result["target_profile"] == "eco-security"
    ...
    def test_non_eco_path_returns_none(self, fake_hermes, monkeypatch, tmp_path):
        _set_active_home(monkeypatch, fake_hermes["security_home"])
        from agent.file_safety import classify_cross_profile_target
        # Path outside any ECO root
        assert classify_cross_profile_target(str(tmp_path / "random.txt")) is None

    def test_eco_config_not_classified_as_cross_profile(self, fake_hermes, monkeypatch):
