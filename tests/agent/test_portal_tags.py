...
def test_eco_client_tag_includes_current_version():
    """The client tag must reflect eco_cli.__version__ verbatim."""
    from eco_cli import __version__
    from agent.portal_tags import eco_client_tag

    assert eco_client_tag() == f"client=eco-client-v{__version__}"


def test_eco_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import eco_client_tag

    tag = eco_client_tag()
    assert tag.startswith("client=eco-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import eco_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=eco-agent" in tags
    assert eco_client_tag() in tags
    assert len(tags) == 2
