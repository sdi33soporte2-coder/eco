...
def sample_repo(tmp_path: Path) -> Path:
    ...
    repo.mkdir()
    _git(repo, "init")
    _git(repo, "config", "user.name", "ECO Tests")
...
@pytest.mark.asyncio
async def test_blocks_sensitive_home_and_eco_paths(tmp_path: Path, monkeypatch):
    from agent.context_references import preprocess_context_references_async
    ...
    eco_env.write_text("API_KEY=super-secret\n", encoding="utf-8")
    ...
    result = await preprocess_context_references_async(
        "read @file:.eco/.env and @file:.ssh/id_rsa",
        cwd=tmp_path,
