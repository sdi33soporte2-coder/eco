...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_eco_env(self):
        ...
        # even ~/.eco/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.eco/.env"))
        assert error is not None
...
class TestCacheFileReadBlocking:
    """Internal ECO cache files must remain blocked."""

    def test_hub_index_cache_blocked(self, tmp_path):
        """Hub index-cache reads are blocked."""
        eco_home = tmp_path / ".eco"
        cache = eco_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._eco_home_path", return_value=eco_home):
            error = get_read_block_error(str(cache))
            assert error is not None
            assert "internal ECO cache" in error

    def test_hub_directory_blocked(self, tmp_path):
        """Hub directory reads are blocked."""
        eco_home = tmp_path / ".eco"
        hub = eco_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        eco_home = tmp_path / ".eco"
        cache = eco_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
