...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate ECO_HOME under tmp_path."""
    monkeypatch.setenv("ECO_HOME", str(tmp_path / ".eco"))
    (tmp_path / ".eco").mkdir()

    # Force the constants/image cache helpers to re-read ECO_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("eco_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_eco_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under ECO_HOME — gateway cleanup
