...
The shim (docker/eco-exec-shim.sh, installed at /opt/eco/bin/eco)
exists to prevent the auth.json ownership-mismatch bug where
`docker exec <c> eco login` would write /opt/data/auth.json as
...
failed: ECO is not logged into Nous Portal" on every message.

These tests verify:

1. ``docker exec <c> eco …`` (defaulting to root) gets dropped to the
...
def test_shim_drops_root_to_eco_uid(sleep_container: str) -> None:
    """docker exec defaults to root; the shim should drop to uid 10000.

    We invoke `eco` with a Python-style `-c` shim equivalent — there's no
    pure-eco "print my uid" command, so we use the venv's python directly
    ...
    fact that the venv's `eco` is a console_scripts entry — under the
    ...
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "eco", "config", "set", "_test.shim_marker", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, f"config set failed: stdout={r.stdout!r} stderr={r.stderr!r}"

    # The written file must be owned by eco, not root.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
    ...
    assert r.stdout.strip() == "eco:eco", (
        f"config.yaml owned by {r.stdout.strip()!r}, expected eco:eco. "
        "The shim did not drop privileges before invoking eco."
    )


def test_shim_short_circuits_for_non_root_exec(sleep_container: str) -> None:
    """docker exec --user eco already runs as 10000; shim should be a no-op.
    ...
    r = subprocess.run(
        ["docker", "exec", "--user", "eco", sleep_container,
         "eco", "config", "set", "_test.shim_short_circuit", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, (
        f"docker exec --user eco failed: {r.stderr!r} stdout={r.stdout!r}. "
        "If the shim mis-handled the non-root path, this would fail with EPERM."
    )

    # File still ends up eco:eco — orthogonally confirms uid.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "stat", "-c", "%U:%G", "/opt/data/config.yaml"],
        capture_output=True, text=True, timeout=10,
...
def test_shim_opt_out_strict_truthiness(
    ...
    assert r.stdout.strip() == "eco:eco", (
        ...
        f"file owner is {r.stdout.strip()!r}, expected eco:eco"
    )
...
def test_main_cmd_path_unaffected(built_image: str) -> None:
    ...
    The shim sits at /opt/eco/bin earliest on PATH; main-wrapper.sh
    invokes `s6-setuidgid eco eco <args>` which resolves `eco`
...
def test_e2e_login_then_supervised_gateway_can_read_auth(
    ...
    Pre-shim: ``docker exec <c> eco login`` (root) wrote
    ...
    "ECO is not logged into Nous Portal" on every message.

    We can't do a real OAuth login in a unit test, but we can stand in
    for it by writing the same file shape via `eco config set`-style
    writes — what matters is the *file ownership invariant* downstream
    of `_save_auth_store`. If the shim works, every file the
    `docker exec` path produces is eco-readable.

    Specifically: pretend the operator ran `eco login` (writes
    auth.json) and verify (a) the file exists and (b) it's readable by
    the eco UID. We use `eco auth list` since that touches the
    ...
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "eco", "config", "set", "_test.e2e_marker", "1"],
        capture_output=True, text=True, timeout=30,
    )
    ...
    assert not unreadable, (
        "Files written by `docker exec` are unreadable to the eco user "
