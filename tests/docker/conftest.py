...
Override the image with ``ECO_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("ECO_TEST_IMAGE", "eco-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``ECO_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("ECO_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged eco user
# ---------------------------------------------------------------------------
#
# Background: every ECO runtime path inside the container drops to UID
# 10000 (the ``eco`` user) via ``s6-setuidgid eco``. ``docker exec``
...
# fails (PermissionError swallowed) for eco, so a test that ran as root
...
# helpers below run every probe under ``-u eco`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: eco).
