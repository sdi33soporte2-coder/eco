...
Before the s6 migration, ``docker run nousresearch/eco-agent gateway
...
def test_gateway_run_no_supervise_flag_preserves_legacy_behavior(
    ...
    Three positive assertions confirm we took the pre-s6 path:
      ...
      * The CMD process is a python ``eco gateway run`` invocation
        (not ``sleep infinity``).
...
def test_supervised_gateway_does_not_recurse(
    ...
    gateway slot would never actually run a python ``eco gateway
    ...
    # Count python processes running `eco gateway run`. If the
    ...
    r = _sh(container_name, "ps -eo pid,cmd | grep -v grep | grep -E 'python.*eco.*gateway run' | wc -l")
    assert r.returncode == 0
    n = int(r.stdout.strip() or 0)
    assert n <= 1, (
        f"expected at most one supervised python `eco gateway run` "
    ...
    # CMD process (PID 17 typically). The static `main-eco`
...
def test_supervised_gateway_stdout_reaches_docker_logs(
    ...
    a poor user experience: the iconic "ECO Gateway Starting…"
    ...
    assert "⚕" in combined or "ECO Gateway Starting" in combined, (
    ...
    assert "⚕" in file_contents or "ECO Gateway Starting" in file_contents, (
