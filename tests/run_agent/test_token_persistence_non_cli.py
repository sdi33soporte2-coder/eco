...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    eco_state = ModuleType("eco_state")
    eco_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "eco_state", eco_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "ECO"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "ECO"
