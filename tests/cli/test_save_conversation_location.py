...
Regression: the old implementation wrote ``eco_conversation_<ts>.json``
to the current working directory (CWD). Users who ran /save expected the
file to be discoverable via ``eco sessions browse``, but CWD-resident
snapshots are not indexed in the state DB and are generally invisible.
The fix writes snapshots under ``~/.eco/sessions/saved/`` and prints
...
def eco_home(tmp_path, monkeypatch):
    ...
    if hasattr(eco_constants, "_eco_home_cache"):
        eco_constants._eco_home_cache = None
    return home
...
def test_save_conversation_writes_under_eco_home(eco_home, tmp_path, monkeypatch, capsys):
    """Snapshot must land under ~/.eco/sessions/saved/, not CWD."""
    ...
    cwd_leak = list(work.glob("eco_conversation_*.json"))
    assert not cwd_leak, f"snapshot leaked to CWD: {cwd_leak}"

    # File MUST be under ~/.eco/sessions/saved/
    saved_dir = eco_home / "sessions" / "saved"
    assert saved_dir.is_dir(), "expected saved/ subdirectory to be created"
    files = list(saved_dir.glob("eco_conversation_*.json"))
    ...
    assert "eco --resume 20260101_120000_abc123" in out, out


def test_save_conversation_empty_history_does_nothing(eco_home, capsys):
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "eco_constants"]:
        sys.modules.pop(mod, None)
    import cli
