...
``os.environ["ECO_YOLO_MODE"]``. That env var is captured once at
...
prompt. Only ``eco --yolo`` (process-start env), ``ECO_YOLO_MODE=1``,
and ``eco config set approvals.mode off`` actually bypassed.
...
class TestToggleYoloIsSessionScoped:
    ...
    def test_toggle_yolo_does_not_mutate_env_var(self):
        """Toggling /yolo must not write ``ECO_YOLO_MODE`` — that path is
        ...
        assert os.environ.get("ECO_YOLO_MODE") is None
...
class TestIsSessionYoloActiveHelper:
    ...
    def test_helper_honors_frozen_yolo_mode(self):
        """``eco --yolo`` sets ``ECO_YOLO_MODE`` before tool imports, so
