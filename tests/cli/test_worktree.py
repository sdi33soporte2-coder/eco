...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("eco/eco-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned eco/* and pr-* branches."""

    def test_prunes_orphaned_eco_branch(self, git_repo):
        """eco/eco-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "eco/eco-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "eco/eco-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "eco/eco-deadbeef" in result.stdout
        ...
        assert "eco/eco-deadbeef" in orphaned
        ...
        assert "eco/eco-deadbeef" not in result.stdout
