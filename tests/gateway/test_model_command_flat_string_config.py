...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    monkeypatch.setattr(
        "eco_cli.model_switch.switch_model",
        lambda **kw: _fake_switch_result(),
    )
    # save_config writes to ``get_eco_home() / config.yaml`` — point it here.
    monkeypatch.setattr("eco_constants.get_eco_home", lambda: eco_home)
    monkeypatch.setattr("eco_cli.config.get_eco_home", lambda: eco_home)
    return cfg_path
