"""Tests for _setup_feishu() in eco_cli/gateway.py.
...
def _run_setup_feishu(
    *,
    ...
    with patch("eco_cli.gateway.save_env_value", side_effect=mock_save), \
         patch("eco_cli.gateway.get_env_value", side_effect=mock_get), \
         patch("eco_cli.gateway.prompt_yes_no", side_effect=prompt_yes_no_responses), \
         patch("eco_cli.gateway.prompt_choice", side_effect=prompt_choice_responses), \
         patch("eco_cli.gateway.prompt", side_effect=prompt_responses), \
         patch("eco_cli.gateway.print_info"), \
         patch("eco_cli.gateway.print_success"), \
         patch("eco_cli.gateway.print_warning"), \
         patch("eco_cli.gateway.print_error"), \
         patch("eco_cli.gateway.color", side_effect=lambda t, c: t), \
         patch("gateway.platforms.feishu.qr_register", return_value=qr_result):

        from eco_cli.gateway import _setup_feishu
        _setup_feishu()
