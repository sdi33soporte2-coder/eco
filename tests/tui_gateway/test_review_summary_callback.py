...
def server():
    with patch.dict(
        "sys.modules",
        {
            "eco_constants": MagicMock(
                get_eco_home=MagicMock(return_value="/tmp/eco_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'eco-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'eco-release' patched"
    }
