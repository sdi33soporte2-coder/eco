...
The PID file lives at ``{ECO_HOME}/gateway.pid``.  ECO_HOME defaults to
``~/.eco`` but can be overridden via the environment variable.  This means
separate ECO_HOME directories naturally get separate PID files — a property
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting ECO_HOME."""
    home = get_eco_home()
...
def _get_lock_dir() -> Path:
    ...
    override = os.getenv("ECO_GATEWAY_LOCK_DIR")
    if override:
        return Path(override)
    state_home = Path(os.getenv("XDG_STATE_HOME", Path.home() / ".local" / "state"))
    return state_home / "eco" / _LOCKS_DIRNAME
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the ECO gateway."""
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different ECO_HOME dirs).
    """
...
# enabled in the user's systemd (e.g. ``eco.service`` + ``eco-
# gateway.service``).
#
