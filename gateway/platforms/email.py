"""
Email platform adapter for the ECO gateway.

Allows users to interact with ECO by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "eco" "version" "{_eco_version}" '
            '"vendor" "NousResearch" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "ECO Agent")
        ...
        msg_id = f"<eco-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"
