...
There are two ways to add a platform to the ECO gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.eco/plugins/` (or under `plugins/platforms/`
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `eco gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `eco_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the ECO core.
...
And add it to the `eco-gateway` composite:

```python
"eco-gateway": {
    "includes": [..., "eco-your-platform"]
}
```
...
## 12. Status Display (`eco_cli/status.py`)
...
## 13. Gateway Setup Wizard (`eco_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ eco_cli/ toolsets.py \
  --include="*.py" -l | sort -u
