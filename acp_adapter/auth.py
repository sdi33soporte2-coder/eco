"""ACP auth helpers — detect and advertise ECO authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "eco-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active ECO runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if ECO can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for ECO.
    ...
    not have ECO provider credentials configured yet, so ECO always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate ECO using the currently configured "
