"""CLI entry point for the eco ACP adapter.

Loads environment variables from ``~/.eco/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from ECO_HOME (default ``~/.eco``)."""
    from eco_cli.env_loader import load_eco_dotenv

    eco_home = get_eco_home()
    loaded = load_eco_dotenv(eco_home=eco_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", eco_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="eco-acp",
        description="Run ECO Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print ECO version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
...
def _run_check() -> None:
    ...
    from acp_adapter.server import ECOACPAgent  # noqa: F401

    print("ECO ACP check OK")


def _run_setup() -> None:
    from eco_cli.main import main as eco_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "eco", "model"]
        eco_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``eco postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting eco ACP adapter")
    ...
    import acp
    from .server import ECOACPAgent
