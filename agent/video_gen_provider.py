...
as ``kind: backend``) or ``~/.eco/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``eco tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$ECO_HOME/cache/videos/``, creating parents as needed."""
    from eco_constants import get_eco_home

    path = get_eco_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$ECO_HOME/cache/videos/``.
