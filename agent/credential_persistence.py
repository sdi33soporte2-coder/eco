...
``auth.json``.  They intentionally have no dependency on ``eco_cli.auth`` so
...
# Sources ECO owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and ECO-owned OAuth/device-code state)
