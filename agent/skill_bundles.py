...
Storage
-------
Bundles live in ``~/.eco/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``ECO_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``eco bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under ECO_HOME.

    Honors ``ECO_BUNDLES_DIR`` for tests; falls back to
    ``<ECO_HOME>/skill-bundles``.
    """
    override = os.environ.get("ECO_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_eco_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `eco bundles` CLI subcommand.
# ---------------------------------------------------------------------------
