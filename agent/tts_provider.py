...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into ECO with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.eco/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of ECO
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``eco tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``eco setup`` for
