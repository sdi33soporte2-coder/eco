...
Every ECO request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to ECO Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=eco",
        "client=eco-client-v<__version__>",
    ]

The version is sourced live from ``eco_cli.__version__`` so it auto-aligns
...
``eco_cli.__version__`` is the canonical source of truth.
"""
...
def _eco_version() -> str:
    """Return the current ECO release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``eco_cli`` cannot be imported (should
...
def eco_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=eco-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=eco-client-v{_eco_version()}"
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=eco", eco_client_tag()]
