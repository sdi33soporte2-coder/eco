...
projection into ECO' display, approval bridging, transcript projection into
...
"codex_app_server"`. ECO' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        if spawn_env.get("ECO_KANBAN_TASK"):
            kanban_db = spawn_env.get("ECO_KANBAN_DB")
            kanban_root = (
                ...
                if kanban_db
                    ...
                    "ECO_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("ECO_HOME", os.path.expanduser("~/.eco")),
                        "kanban",
                    ),
                )
            )
