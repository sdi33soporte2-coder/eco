...
Owns one Codex thread per ECO session. Drives `turn/start`, consumes
...
# ECO' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_ECO_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse ECO/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per ECO session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _ECO_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("ECO_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="eco",
            client_title="ECO Agent",
            client_version=_get_eco_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but ECO' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into ECO' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own eco-tools callback we
            # auto-accept — the user already approved ECO' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map ECO approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    ECO returns 'once', 'session', 'always', or 'deny'.
...
def _get_eco_version() -> str:
    """Best-effort ECO version string for codex's userAgent line."""
    try:
