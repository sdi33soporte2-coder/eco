...
appropriate.  All installs go to a ECO-owned bin staging dir,
``<ECO_HOME>/lsp/bin/``, so we don't pollute the user's global
toolchain.
...
- ``manual`` — never install; if a binary is missing, the server is
  silently skipped and the user is told about it via ``eco lsp
  status``.
...
# ``<ECO_HOME>/lsp/bin/`` first, then on PATH.
#
# Optional fields:
...
def eco_lsp_bin_dir() -> Path:
    """Return the ECO-owned bin staging dir for LSP servers."""
    home = os.environ.get("ECO_HOME")
    if home is None:
        home = os.path.join(os.path.expanduser("~"), ".eco")
    p = Path(home) / "lsp" / "bin"
...
def _install_npm(
    ...
    if npm is None:
        ...
        return None
    staging = eco_lsp_bin_dir().parent  # <ECO_HOME>/lsp/
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    """Install a Python package into a eco-owned target dir.
    ...
    console script.
    """
    pip_target = eco_lsp_bin_dir().parent / "python-packages"
...
def detect_status(pkg: str) -> str:
    ...
    Used by the ``eco lsp status`` CLI to give users a quick
