...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time ECO starts and finds ``~/.openclaw/``.

    Points users at ``eco claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``eco claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to ECO, run "
        "`eco claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `eco claw cleanup` (renames it to ~/.openclaw.pre-migration — "
