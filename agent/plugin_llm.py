...
Plugins built on ECO Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing ECO
...
:class:`~eco_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors ECO' main config
...
handles every provider, fallback chain, and per-task override ECO
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`eco_cli.plugins.PluginContext`
