...
React + Ink terminal UI for ECO. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
- Input history is stored in `~/.eco/.eco_history` or under `HERMES_HOME`.
...
```text
ui-tui/
  packages/eco-ink/   forked Ink renderer (local dep)
  src/
    ...
    types/
      eco-ink.d.ts    type declarations for @eco/ink
