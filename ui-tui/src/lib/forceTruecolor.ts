...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.ECO_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep ECO on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via ECO_TUI_TRUECOLOR=1.
