import type { RunExternalProcess } from '@eco/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`eco ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching eco: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`eco ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
