import type { MouseTrackingMode } from '@eco/ink'
...
export const STARTUP_RESUME_ID = (process.env.ECO_TUI_RESUME ?? '').trim()
export const STARTUP_QUERY = (process.env.ECO_TUI_QUERY ?? '').trim()
export const STARTUP_IMAGE = (process.env.ECO_TUI_IMAGE ?? '').trim()
...
//
// - ECO_TUI_MOUSE_TRACKING (truthy/falsy) explicitly overrides everything.
//   This is the "force a value" knob and intentionally beats the legacy
//   kill-switch and the Termux default.
// - ECO_TUI_DISABLE_MOUSE=1 forces mouse off — the legacy kill switch.
...
//   behavior.
const mouseTrackingOverride = parseToggle(process.env.ECO_TUI_MOUSE_TRACKING)
const mouseTrackingDisabledLegacy = truthy(process.env.ECO_TUI_DISABLE_MOUSE)
...
export const NO_CONFIRM_DESTRUCTIVE = truthy(process.env.ECO_TUI_NO_CONFIRM)

const inlineOverride = parseToggle(process.env.ECO_TUI_INLINE)
...
// less fragile. Override explicitly with ECO_TUI_INLINE=0/1.
...
// synthetic timer).
export const SHOW_FPS = truthy(process.env.ECO_TUI_FPS)
