...
describe('statusRuleWidths', () => {
  it('keeps the status rule within the terminal width', () => {
    for (const cols of [8, 12, 20, 40, 100]) {
      const widths = statusRuleWidths(cols, '~/src/eco/main (some-long-branch-name)')
  ...
  it('truncates the cwd segment before it can wrap in skinny terminals', () => {
    const widths = statusRuleWidths(24, '~/src/eco/main (bb/some-extremely-long-branch)')

    expect(widths.rightWidth).toBeLessThan('~/src/eco/main (bb/some-extremely-long-branch)'.length)
