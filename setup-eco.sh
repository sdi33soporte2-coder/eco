...
# 5. Symlinks the 'eco' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ ECO Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/eco-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink eco into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up eco command..."

HERMES_BIN="$SCRIPT_DIR/venv/bin/eco"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$HERMES_BIN" "$COMMAND_LINK_DIR/eco"
echo -e "${GREEN}✓${NC} Symlinked eco → $COMMAND_LINK_DISPLAY_DIR/eco"
...
# ============================================================================
# Seed bundled skills into ~/.eco/skills/
# ============================================================================

HERMES_SKILLS_DIR="${HERMES_HOME:-$HOME/.eco}/skills"
mkdir -p "$HERMES_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.eco/skills/ ..."
...
echo "Other commands:"
echo "  eco status        # Check configuration"
if is_termux; then
    echo "  eco gateway       # Run gateway in foreground"
else
    echo "  eco gateway install # Install gateway service (messaging + cron)"
fi
echo "  eco cron list     # View scheduled jobs"
echo "  eco doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m eco_cli.main setup
fi
