"""Persistent slash-command worker — one ECOCLI per TUI session.
...
from cli import ECOCLI
from rich.console import Console


def _run(cli: ECOCLI, command: str) -> str:
...
def main():
    ...
    os.environ["ECO_SESSION_KEY"] = args.session_key
    os.environ["ECO_INTERACTIVE"] = "1"

    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = ECOCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
