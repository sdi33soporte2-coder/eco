/**
 * ECO Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.eco/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__ECO_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__ECO_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "eco-kanban-dot-triage",
    todo: "eco-kanban-dot-todo",
    ready: "eco-kanban-dot-ready",
    running: "eco-kanban-dot-running",
    blocked: "eco-kanban-dot-blocked",
    done: "eco-kanban-dot-done",
    archived: "eco-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-eco-task";
  ...
  const DOCS_URL = "https://eco.nousresearch.com/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://eco.nousresearch.com/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "eco.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="eco-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "eco-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "eco-kanban-md",
  ...
  // standard `drop` event and our `eco-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("eco-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("eco-kanban-column--drop");
          if (target) target.classList.add("eco-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("eco-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("eco-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("eco-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        const token = window.__ECO_SESSION_TOKEN__ || "";
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "eco-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "eco-kanban-attention",
        "eco-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "eco-kanban-attention-bar" },
        h("span", { className: "eco-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "eco-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "eco-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "eco-kanban-attention-row",
                  "eco-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "eco-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "eco-kanban-attention-row-id" }, task.id),
                h("span", { className: "eco-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "eco-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "eco-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "eco-kanban-diag-action-btn",
      action.suggested ? "eco-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " eco-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".eco-kanban-drawer-comment-row input, .eco-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "eco-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("eco-kanban-diag", sevClass) },
      h("div", { className: "eco-kanban-diag-header" },
        h("span", { className: "eco-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "eco-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "eco-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "eco-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "eco-kanban-diag-data-row" },
                  h("span", { className: "eco-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "eco-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "eco-kanban-diag-data-row" },
                h("span", { className: "eco-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "eco-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "eco-kanban-diag-reassign-row" },
            h("span", { className: "eco-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "eco-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "eco-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "eco-kanban-diag-msg",
              msg.ok ? "eco-kanban-diag-msg--ok" : "eco-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "eco-kanban-section" },
      h("div", { className: "eco-kanban-section-head-row" },
        h("span", { className: "eco-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "eco-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "eco-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "eco-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "eco-kanban-docs-link",
      title: "Open ECO Kanban docs in a new tab",
      "aria-label": "ECO Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `eco kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "eco-kanban-msg-ok" : "eco-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "eco-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "eco-kanban-boardswitcher" },
      h("div", { className: "eco-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "eco-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "eco-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "eco-kanban-dialog-title" },
        ...
        h("div", { className: "eco-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned ECO profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "eco-kanban-bulk" },
      h("span", { className: "eco-kanban-bulk-count" },
      ...
      h("div", { className: "eco-kanban-bulk-priority",
      ...
      h("div", { className: "eco-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different ECO profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "eco-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("eco-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("eco-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "eco-kanban-trash",
        dragOver ? "eco-kanban-trash--drop" : "",
        props.draggingTaskId ? "eco-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "eco-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "eco-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".eco-kanban-card");
      if (!card) return;
    ...
    return h("div", { className: "eco-kanban-columns", onDragStart: handleDragStart, onDragEnd: handleDragEnd },
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("eco-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("eco-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "eco-kanban-column",
        dragOver ? "eco-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "eco-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "eco-kanban-col-check",
        ...
        h("span", { className: cn("eco-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "eco-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "eco-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "eco-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "eco-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "eco-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "eco-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "eco-kanban-lane" },
                  h("div", { className: "eco-kanban-lane-head" },
                    h("span", { className: "eco-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "eco-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "eco-kanban-card--stale-red";
    if (age >= tier.amber) return "eco-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".eco-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "eco-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "eco-kanban-card",
        props.selected ? "eco-kanban-card--selected" : "",
        props.failed ? "eco-kanban-card--failed" : "",
        props.draggingSource ? "eco-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "eco-kanban-card-content" },
          h("div", { className: "eco-kanban-card-row" },
            h("label", {
              className: "eco-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "eco-kanban-card-check",
            ...
            h("span", { className: "eco-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or eco kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "eco-kanban-warning-badge",
                    "eco-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            progress
              ? h("span", {
                  className: cn(
                    "eco-kanban-progress",
                    progress.done === progress.total ? "eco-kanban-progress--full" : "",
                  ),
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "eco-kanban-needs-assignee",
          ...
          h("div", { className: "eco-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "eco-kanban-card-row eco-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "eco-kanban-assignee",
                            title: `Assigned to ECO profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "eco-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "eco-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "eco-kanban-ago",
  ...
  function InlineCreate(props) {
    ...
    return h("div", { className: "eco-kanban-inline-create" },
      h("textarea", {
        value: title,
      ...
      h("div", { className: "flex gap-2" },
        h(Input, {
          ...
          title: props.columnName === "triage"
            ? "ECO profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
            : "ECO profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "eco-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "eco-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "eco-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "eco-kanban-drawer-close",
        ...
        data ? h("div", { className: "eco-kanban-drawer-comment-row" },
          h(Input, {
            value: newComment,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "eco-kanban-drawer-body" },
      h("div", { className: "eco-kanban-drawer-title" },
        h("span", { className: cn("eco-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
      ...
      t.result ? h("div", { className: "eco-kanban-section" },
        h("div", { className: "eco-kanban-section-head" }, tx(i18n, "result", "Result")),
        h(MarkdownBlock, { source: t.result, enabled: props.renderMarkdown }),
      ) : null,
      h("div", { className: "eco-kanban-section" },
        h("div", { className: "eco-kanban-section-head" },
          `${tx(i18n, "comments", "Comments")} (${comments.length})`),
        comments.length === 0
          ? h("div", { className: "text-xs text-muted-foreground" },
              tx(i18n, "noComments", "— no comments —"))
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "eco-kanban-comment" },
                h("div", { className: "eco-kanban-comment-head" },
                  h("span", { className: "eco-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "eco-kanban-comment-ago" },
      ...
      h("div", { className: "eco-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            ...
            className: cn(
              "eco-kanban-event",
              isDiag ? "eco-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "eco-kanban-event-header" },
                  h("span", { className: "eco-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "eco-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "eco-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "eco-kanban-event-header-plain" },
                  h("span", { className: "eco-kanban-event-kind" }, e.kind),
                  h("span", { className: "eco-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "eco-kanban-event-phantom-row" },
                  h("span", { className: "eco-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "eco-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "eco-kanban-event-payload" },
    ...
    return h("div", { className: "eco-kanban-section" },
      h("div", { className: "eco-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "eco-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `eco-kanban-run--${r.outcome || r.status || "ended"}`
          : "eco-kanban-run--active";
        return h("div", { key: r.id, className: cn("eco-kanban-run", outcomeClass) },
          h("div", { className: "eco-kanban-run-head" },
            h("span", { className: "eco-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "eco-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "eco-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "eco-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "eco-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "eco-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "eco-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "eco-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "eco-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "eco-kanban-pre eco-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "eco-kanban-meta-row" },
      h("span", { className: "eco-kanban-meta-label" }, props.label),
      h("span", { className: "eco-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "eco-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "eco-kanban-meta-row" },
        h("span", { className: "eco-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "eco-kanban-meta-value eco-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "eco-kanban-meta-row" },
        h("span", { className: "eco-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "eco-kanban-meta-value eco-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "eco-kanban-section" },
      h("div", { className: "eco-kanban-section-head-row" },
        h("span", { className: "eco-kanban-section-head" }, tx(t, "description", "Description")),
        editing
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "eco-kanban-section" },
      h("div", { className: "eco-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "eco-kanban-deps-row" },
        h("span", { className: "eco-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "eco-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "eco-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "eco-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "eco-kanban-dep-chip-x",
      ...
      h("div", { className: "eco-kanban-deps-row" },
        h("span", { className: "eco-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "eco-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "eco-kanban-actions" },
        specifyButton,
        decomposeButton,
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "eco-kanban-section" },
      ...
      h("div", { className: "eco-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "eco-kanban-home-sub eco-kanban-home-sub--on"
              : "eco-kanban-home-sub",
          }, label);
        })
      )
  ...
  if (window.__ECO_PLUGINS__ && typeof window.__ECO_PLUGINS__.register === "function") {
    window.__ECO_PLUGINS__.register("kanban", KanbanPage);
  }
})();
