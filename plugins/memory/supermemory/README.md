...
```bash
eco memory setup    # select "supermemory"
```

Or manually:

```bash
eco config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.eco/.env
```
...
|-----|---------|-------------|
| `container_tag` | `eco` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `eco-{identity}` → `eco-coder`). |
...
Use `{identity}` in the `container_tag` to scope memories per ECO profile:

```json
{
  "container_tag": "eco-{identity}"
}
```

For a profile named `coder`, this resolves to `eco-coder`. The default profile resolves to `eco-default`. Without `{identity}`, all profiles share the same container.
