...
def _load_supermemory_config(eco_home: str) -> dict:
    config = _default_config()
    config_path = Path(eco_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, eco_home: str) -> None:
    config_path = Path(eco_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `eco memory setup`.
        # All other options are documented for $ECO_HOME/supermemory.json
    ...
    def save_config(self, values, eco_home):
        ...
        _save_supermemory_config(sanitized, eco_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from eco_constants import get_eco_home
        self._eco_home = kwargs.get("eco_home") or str(get_eco_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._eco_home)
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        metadata = {"source": "eco", "type": "conversation_turn"}
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "eco_memory", "target": target, "type": "explicit_memory"},
