...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/eco>
...
```bash
eco honcho setup    # full interactive wizard (cloud or local)
eco memory setup    # generic picker, also works
```

Or manually:
```bash
eco config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.eco/.env
```
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$HERMES_HOME/honcho.json` | Profile-local (isolated ECO instances) |
| 2 | `~/.eco/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active ECO profile: `eco` (default) or `eco.<profile>`.
...
**Deployment shapes** (`eco honcho setup` asks one prompt to set these):

- **Single-operator** — `pinUserPeer: true`. All gateway users → `peerName`. Recommended for personal use where you connect ECO to your own Telegram/Discord/etc.
...
| 4 | `per-session` strategy | ECO session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`eco-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`eco`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-eco-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening eco in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — ECO session ID (timestamp + hex). Every `eco` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple ECO profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`eco`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active ECO profile: `eco` (default) or `eco.<profile>` (e.g. `eco -p coder` → host key `eco.coder`).
...
| Command | Description |
|---------|-------------|
| `eco honcho setup` | Full interactive setup wizard |
| `eco honcho status` | Show resolved config for active profile |
| `eco honcho enable` / `disable` | Toggle Honcho for active profile |
| `eco honcho mode <mode>` | Change recall or observation mode |
| `eco honcho peer --user <name>` | Update user peer name |
| `eco honcho peer --ai <name>` | Update AI peer name |
| `eco honcho tokens --context <N>` | Set context token budget |
| `eco honcho tokens --dialectic <N>` | Set dialectic max chars |
| `eco honcho map <name>` | Map current directory to a session name |
| `eco honcho sync` | Create host blocks for all ECO profiles |
