"""SimpleX Chat platform adapter (ECO plugin).
...
``plugins/platforms/simplex/``. The ECO plugin loader scans the
...
discoverable and `eco setup` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside ECO — no
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        payload = {
            "corrId": f"eco-snd-{int(time.time() * 1000)}",
            "cmd": cmd_str,
        }
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``eco setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / home channel.
    Writes to ``~/.eco/.env`` via ``eco_cli.config``.
    """
    print()
    ...
    try:
        ...
        print("eco_cli.config not available; set SIMPLEX_* vars manually in ~/.eco/.env")
        return
