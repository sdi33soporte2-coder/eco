...
# ``model_tools`` import time, so ``eco status``, ``eco chat``, etc.
...
# ``eco_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "ECO is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a eco MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${ECO_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _eco_home = _get_eco_home()
        except (ModuleNotFoundError, ImportError):
            _eco_home = _Path.home() / ".eco"
        self._thread_count_store = _ThreadCountStore(
            _eco_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "ECO is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("ECO_HOME", str(_Path.home() / ".eco"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${ECO_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a eco MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'ECO is thinking…' marker message.
        ...
        "ECO is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "ECO is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "ECO is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "ECO is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "ECO is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``eco setup``.

    The setup wizard at ``eco_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.eco/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. eco-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.eco/.env")
    print_info("Restart the gateway: eco gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``eco cron`` running as a
    separate process from ``eco gateway``).  Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'eco[google_chat]'",
        ...
        # the "ECO is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'ECO is thinking…' marker message "
