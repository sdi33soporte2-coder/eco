"""
Microsoft Teams platform adapter for ECO Agent.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "ECO"}),
            )
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        eco_action = data.get("eco_action", "")
        session_key = data.get("session_key", "")

        if not eco_action or not session_key:
        ...
        choice = choice_map.get(eco_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
        ...
        card = (
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="eco_approve",
                    data={**btn_data_base, "eco_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="eco_approve",
                    data={**btn_data_base, "eco_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="eco_approve",
                    data={**btn_data_base, "eco_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="eco_approve",
                    data={**btn_data_base, "eco_action": "deny"},
                    style="destructive",
                ),
            ])
        )
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"ECO\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
    ...
    print()
    print_success("Teams configuration saved to ~/.eco/.env")
