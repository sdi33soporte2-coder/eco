"""
LINE Messaging API platform adapter for ECO Agent.
...
to ECO Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``ECO_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            eco_home = Path.home().joinpath(".eco").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``eco status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``eco status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``eco setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.eco/.env`` via ``eco_cli.config``.
    """
    print()
    ...
    try:
        from eco_cli.config import get_env_var, set_env_var
    except ImportError:
        print("eco_cli.config not available; set LINE_* vars manually in ~/.eco/.env")
        return
