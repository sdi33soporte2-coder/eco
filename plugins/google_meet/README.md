...
Let the eco agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where eco runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `eco meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `eco meet` CLI |
...
| `cli.py` | `eco meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/cli.py` | v3: `eco meet node {run,list,approve,remove,status,ping}` |
...
eco meet install                                      # pip + Chromium
eco meet setup                                        # preflight
eco meet auth                                         # optional
eco meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
eco meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.eco/.env
eco meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
eco meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
eco meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.eco/.env
eco meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, eco will **not** switch your system audio input automatically — the
...
eco plugins enable google_meet
eco meet node run --display-name my-mac --host 0.0.0.0 --port 18789
