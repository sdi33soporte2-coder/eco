...
session-scoped state file under ``$ECO_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $ECO_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_eco_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this eco install, leave it first —
    ...
    env = os.environ.copy()
    env["ECO_MEET_URL"] = url
    env["ECO_MEET_OUT_DIR"] = str(out)
    env["ECO_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["ECO_MEET_HEADED"] = "1"
    if auth_state:
        env["ECO_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["ECO_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if ECO_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["ECO_MEET_MODE"] = mode
    if realtime_model:
        env["ECO_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["ECO_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["ECO_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["ECO_MEET_REALTIME_KEY"] = realtime_api_key
