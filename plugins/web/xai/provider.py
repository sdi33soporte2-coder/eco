...
``{title, url, description, position}`` rows every other ECO web
provider produces.
...
prefers ECO-managed xAI Grok OAuth (via ``eco auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.eco/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``eco tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `eco auth` to sign in with "
