"""disk_cleanup — ephemeral file cleanup for ECO Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under ECO_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly ECO_HOME and /tmp/eco-*
Never touches: ~/.eco/logs/ or any system directory.
"""
...
try:
    ...
    def get_eco_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("ECO_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".eco").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$ECO_HOME/logs/``."""
    return get_eco_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$ECO_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under ECO_HOME or ``/tmp/eco-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    eco_home = get_eco_home()
    try:
        path.resolve().relative_to(eco_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/eco-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("eco-"):
        return True
    return False
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside ECO_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under ECO_HOME (but leave ECO_HOME itself and
    ...
    _PROTECTED_TOP_LEVEL = {
        ...
        "eco", "backups", "profiles", ".worktrees",
    }
    empty_removed = 0
    try:
        for dirpath in sorted(eco_home.rglob("*"), reverse=True):
            if not dirpath.is_dir() or dirpath == eco_home:
                continue
            try:
                rel_parts = dirpath.relative_to(eco_home).parts
            except ValueError:
                continue
...
def guess_category(path: Path) -> Optional[str]:
    ...
    eco_home = get_eco_home()
    try:
        rel = path.resolve().relative_to(eco_home)
        ...
        # Path isn't under ECO_HOME (e.g. /tmp/eco-*) — fall through.
        pass
