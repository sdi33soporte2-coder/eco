#!/usr/bin/env bash
# Canonical test runner for eco-agent. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.eco/...)
#
# Usage:
...
# ── Activate venv ───────────────────────────────────────────────────────────
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.eco/eco-agent/venv"; do
...
if [ -f "$HOME/.eco/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.eco"
