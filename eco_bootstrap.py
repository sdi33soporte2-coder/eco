"""Windows UTF-8 bootstrap for ECO entry points.
...
should be imported at the very top of every ECO entry point
(``eco``, ``eco``, ``eco-acp``, ``python -m gateway.run``,
...
# Apply on import — entry points just need ``import eco_bootstrap``
# (or ``from eco_bootstrap import apply_windows_utf8_bootstrap``) at
