...
Usage:
    ...
    python cli.py --skills eco-agent-dev,github-auth
...
try:
    from eco_cli.pt_input_extras import (
...
class HermesCLI:
    ...
    def _is_session_yolo_active(self) -> bool:
        ...
        ``ECO_YOLO_MODE`` into ``_YOLO_MODE_FROZEN`` before tool imports
        happen.
        """
        try:
            from tools.approval import (
    ...
    def _toggle_yolo(self):
        ...
        ``ECO_YOLO_MODE`` here — that env var is read once at module import
    ...
    def run(self):
        ...
        def handle_ignored_terminal_sequence(event):
            ...
            install_ignored_terminal_sequences() in eco_cli.pt_input_extras
...
def main(
    ...
    if query or image:
        ...
        # Kanban workers spawn with ``eco chat -q "work kanban task <id>"``;
        ...
        _kanban_task_id = os.environ.get("ECO_KANBAN_TASK", "").strip()
        if _kanban_task_id:
            try:
