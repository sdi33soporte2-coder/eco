...
# by /etc/cont-init.d/01-eco-setup (installed by the Dockerfile). This
...
# on the pre-s6 contract "entrypoint.sh sets up state then execs eco"
...
echo "[eco] WARNING: docker/entrypoint.sh is a deprecated shim under " \
...
exec /opt/eco/docker/stage2-hook.sh "$@"
