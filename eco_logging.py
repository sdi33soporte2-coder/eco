"""Centralized logging setup for ECO Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.eco/logs/`` (profile-aware via ``get_eco_home()``).
...
from eco_constants import get_config_path, get_eco_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_eco_session_injector", False):
    ...
    _session_record_factory._eco_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``eco logs --component``.
COMPONENT_PREFIXES = {
    "gateway": ("gateway", "eco_plugins"),
...
def setup_logging(
    *,
    ...
    """Configure the ECO logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    eco_home
        Override for the ECO home directory.  Falls back to
        ``get_eco_home()`` (profile-aware).
    log_level
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_eco_verbose", False):
                return

    handler = logging.StreamHandler()
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._eco_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    inherit the eco group. However, both _open() (initial creation) and
    ...
    def __init__(self, *args, **kwargs):
        from eco_cli.config import is_managed
