#!/usr/bin/env python3
"""Google Workspace API CLI for ECO Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing ECO-facing JSON contract and falls back to the Python client
...
from _eco_home import get_eco_home
...
def _gws_binary() -> str | None:
    override = os.getenv("ECO_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for ECO Agent")
