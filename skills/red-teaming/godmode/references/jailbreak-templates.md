...
## 5. GODMODE FAST — ECO 4 405B (Zero Refusal)

**Model:** `nousresearch/eco-4-405b`
**Strategy:** ECO models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with ECO Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.eco/config.yaml`:
