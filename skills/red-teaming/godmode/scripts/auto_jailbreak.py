...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("ECO_HOME", os.path.expanduser("~/.eco")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("ECO_HOME", Path.home() / ".eco")) / "skills" / "red-teaming" / "godmode"
...
ECO_HOME = Path(os.getenv("ECO_HOME", Path.home() / ".eco"))
CONFIG_PATH = ECO_HOME / "config.yaml"
PREFILL_PATH = ECO_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/ECO models — already uncensored, just needs clean prompt
    "eco": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "eco" in model_lower or "nous" in model_lower:
        return "eco"
...
def _get_current_model() -> tuple:
    """Read current model and provider from ECO config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.eco/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart ECO for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart ECO for changes to take effect.")
