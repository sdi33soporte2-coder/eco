...
When running ECO inside Docker, the default `network_mode: host` gives the
...
The ECO [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=eco,eco-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://eco-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — ECO trust model and vulnerability reporting
