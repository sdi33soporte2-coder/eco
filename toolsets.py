...
_ECO_CORE_TOOLS = [
    ...
    # spawned as a kanban worker (ECO_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `eco tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (ECO_KANBAN_TASK env "
    ...
    # ==========================================================================
    # Full ECO toolsets (CLI + messaging platforms)
    #
    ...
    "eco-cli": {
        ...
        "tools": _ECO_CORE_TOOLS,
        "includes": []
    },

    "eco-cron": {
        # Mirrors eco-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `eco tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as eco-cli; gated by `eco tools`",
        "tools": _ECO_CORE_TOOLS,
        "includes": []
    },
    ...
    "eco-email": {
        "description": "Email bot toolset - interact with ECO via email (IMAP/SMTP)",
        "tools": _ECO_CORE_TOOLS,
        "includes": []
    },
    ...
    "eco-sms": {
        "description": "SMS bot toolset - interact with ECO via SMS (Twilio)",
        "tools": _ECO_CORE_TOOLS,
        "includes": []
    },
    ...
    "eco-gateway": {
        ...
        "tools": [],
        "includes": ["eco-telegram", "eco-discord", "eco-whatsapp", "eco-slack", "eco-signal", "eco-bluebubbles", "eco-homeassistant", "eco-email", "eco-sms", "eco-mattermost", "eco-matrix", "eco-dingtalk", "eco-feishu", "eco-wecom", "eco-wecom-callback", "eco-weixin", "eco-qqbot", "eco-webhook", "eco-yuanbao"]
    }
}
...
def resolve_toolset(name: str, visited: Set[str] = None) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (eco-<name>).
        # Gives them _ECO_CORE_TOOLS plus any tools the plugin registered
        # into a toolset matching the platform name.
        if name.startswith("eco-"):
            platform_name = name[len("eco-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_ECO_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
