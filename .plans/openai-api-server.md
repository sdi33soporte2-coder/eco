# OpenAI-Compatible API Server for ECO Agent
...
SSE streaming. By exposing this endpoint, eco-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  eco-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.eco/.env`
2. Run `eco gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with eco-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns eco-agent as a model) |
...
Authorization: Bearer eco-api-key-here
...
But eco-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "eco-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured eco-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of eco-agent's core prompt
...
Once implemented, eco-agent works as a drop-in backend for:
