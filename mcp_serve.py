"""
ECO MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (ECO-specific extra)

Usage:
    eco mcp serve
    eco mcp serve --verbose
...
logger = logging.getLogger("eco.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using ECO_HOME."""
    try:
        from eco_constants import get_eco_home
        return get_eco_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("ECO_HOME", Path.home() / ".eco")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from eco_constants import get_eco_home
        directory_file = get_eco_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("ECO_HOME", Path.home() / ".eco")
...
class EventBridge:
    ...
    This is the ECO equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from eco_constants import get_eco_home
            db_file = get_eco_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("ECO_HOME", Path.home() / ".eco")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the ECO MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "eco",
        instructions=(
            "ECO Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the ECO MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
