...
Pure logic: walks a ECO config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `eco doctor` and a future `eco migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a ECO config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a ECO config and return retirement issues.
