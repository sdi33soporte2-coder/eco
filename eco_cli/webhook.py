"""eco webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    eco webhook subscribe <name> [options]
    eco webhook list
    eco webhook remove <name>
    eco webhook test <name> [--payload '{"key": "value"}']

Subscriptions persist to ~/.eco/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _eco_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: eco gateway run
"""
...
def webhook_command(args):
    """Entry point for 'eco webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: eco webhook {subscribe|list|remove|test}")
        print("Run 'eco webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (eco gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: eco webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from eco webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (eco gateway run)")
