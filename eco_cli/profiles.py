"""
Profile management for multiple isolated ECO instances.

Each profile is a fully independent ECO_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.eco/profiles/<name>/`` by default.

The "default" profile is ``~/.eco`` itself — backward compatible,
zero migration needed.

Usage::

    eco profile create coder          # fresh profile + bundled skills
    eco profile create coder --clone  # also copy config, .env, SOUL.md, skills
    eco profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    eco -p coder chat                 # or via flag
    eco profile use coder             # set as sticky default
    eco profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See eco_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
# Marker file written by `eco profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `eco
...
# `eco skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Two categories:
      1. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         ECO infrastructure directories that only the default profile
         (``~/.eco``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_eco_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.eco) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "eco",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "eco", "default", "test", "tmp", "root", "sudo",
})

# ECO subcommands that cannot be used as profile names/aliases
_ECO_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the eco root, NOT to the current ECO_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where ECO_HOME points outside
    ``~/.eco``, profiles live under ``ECO_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_eco_home() / "profiles"


def _get_default_eco_home() -> Path:
    """Return the default (pre-profile) ECO_HOME path.

    In standard deployments this is ``~/.eco``.
    In Docker/custom deployments where ECO_HOME is outside ``~/.eco``
    (e.g. ``/opt/data``), returns ECO_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_eco_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the ECO installation itself or a common system binary.  "
            f"Pick a different name."
        )


def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its ECO_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_eco_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, eco subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
    if canon in _RESERVED_NAMES:
        return f"'{canon}' is a reserved name"
    if canon in _ECO_SUBCOMMANDS:
        return f"'{canon}' conflicts with a eco subcommand"
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``eco profile list`` for an unrelated profile.
    """
...
# ``config.yaml``. ``config.yaml`` is the user-facing ECO config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_eco_home()
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``eco update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.eco)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.eco/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Write the opt-out marker so seed_profile_skills() and `eco update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`eco profile create --no-skills`).\n"
                "Delete this file to re-enable sync on the next `eco update`.\n",
                encoding="utf-8",
            )
        except OSError:
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `eco profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `eco -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches ECO_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``eco profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "ECO_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.eco).\n"
            "To remove everything, use: eco uninstall"
        )
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    try:
        ...
        from eco_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set ECO_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("ECO_HOME")
    try:
        os.environ["ECO_HOME"] = str(profile_dir)
        from eco_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["ECO_HOME"] = old_home
        elif "ECO_HOME" in os.environ:
            del os.environ["ECO_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.eco/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: eco profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from ECO_HOME.

    Returns ``"default"`` if ECO_HOME is not set or points to ``~/.eco``.
    Returns the profile name if ECO_HOME points into ``~/.eco/profiles/<name>``.
    Returns ``"custom"`` if ECO_HOME is set to an unrecognized path.
    """
    from eco_constants import get_eco_home
    eco_home = get_eco_home()
    resolved = eco_home.resolve()

    default_resolved = _get_default_eco_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.eco itself — its parent is ~/ and its
        # directory name is ".eco", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: eco profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.eco itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.eco). "
            "Specify a different name: eco profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="eco_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"eco.{old_name}"
    new_host = f"eco.{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_eco_home() / "honcho.json",
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a ECO_HOME path string.

    Called early in the CLI entry point, before any eco modules
    are imported, to set the ECO_HOME environment variable.
    """
