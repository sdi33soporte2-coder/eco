"""Profile distributions — shareable, packaged ECO profiles via git.

A distribution is a ECO profile published as a git repository (or
...
* ``eco profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``eco skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``eco profile``, not a parallel tree):

    eco profile install <source> [--name N] [--alias] [--force] [--yes]
    eco profile update  <name>  [--force-config] [--yes]
    eco profile info    <name>
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "eco", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            eco_requires=str(data.get("eco_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.eco_requires:
            out["eco_requires"] = self.eco_requires
        if self.author:
...
def check_eco_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires ECO {op}{target}, "
...
def _env_template_from_manifest(manifest: DistributionManifest) -> str:
    ...
    lines = [
        "# Environment variables required by this ECO distribution.",
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``eco profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a ECO profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a ECO distribution."
        )

    # Version check up-front so we fail fast
    check_eco_requires(manifest.eco_requires, eco_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.eco).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="eco_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `eco profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`eco profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="eco_dist_update_") as tmp:
        plan = plan_install(
