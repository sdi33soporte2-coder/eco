...
on Japanese locales, etc.).  ECO's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``eco_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. ECO is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``ECO_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("ECO_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the ECO-managed Git install directories so
    ...
    # already-running shells don't see the change — which means eco
    ...
    # install without ECO-managed PortableGit).
    ...
    # stdin is re-configured for completeness; ECO's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and ECO's
    ``eco config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching ECO (User env var in Windows Settings, or
    export in a PowerShell profile) and ECO picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known ECO-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\eco\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``eco`` from right after install) retain their old PATH.

    Any subprocess ECO spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known ECO-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "eco", "git", "cmd"),
        os.path.join(local_appdata, "eco", "git", "bin"),
        os.path.join(local_appdata, "eco", "git", "usr", "bin"),
        # ECO venv Scripts directory — host of the eco.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes eco, but harmless to include.
        os.path.join(local_appdata, "eco", "eco", "venv", "Scripts"),
