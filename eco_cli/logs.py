"""``eco logs`` — view and filter ECO log files.
...
Usage examples::
    ...
    eco logs                    # last 50 lines of agent.log
    eco logs -f                 # follow agent.log in real time
    eco logs errors             # last 50 lines of errors.log
    eco logs gateway -n 100    # last 100 lines of gateway.log
    eco logs --level WARNING    # only WARNING+ lines
    eco logs --session abc123   # filter by session ID substring
    eco logs --component tools  # only tool-related lines
    eco logs --since 1h         # lines from the last hour
    eco logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_eco_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when ECO runs — try 'eco chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_eco_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_eco_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_eco_home()}/logs/")
        return

    print(f"Log files in {display_eco_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'eco chat' to generate logs)")
