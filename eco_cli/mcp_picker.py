"""MCP picker — interactive `eco mcp picker` (also the default `eco mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``eco mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `eco plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from eco_cli.cli_output import prompt_yes_no
from eco_cli.curses_ui import curses_single_select
from eco_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new ECO session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from eco_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `eco mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: eco mcp install <name>    Picker: eco mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their ECO is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer ECO — run `eco update` "
...
def show_catalog() -> None:
    """`eco mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`eco mcp picker` (and default `eco mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`eco mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `eco mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
