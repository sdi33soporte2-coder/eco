"""CLI subcommand: ``eco send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(f"eco send: cannot read {file_path}: {exc}", file=sys.stderr)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"eco send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"eco send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"eco send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"eco send: no targets found for platform '{platform_filter}'. "
    ...
    if not any(platforms.values()):
        ...
        print("Set one up with `eco gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.eco/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_eco_env() -> None:
    """Populate ``os.environ`` from ``~/.eco/.env`` AND bridge top-level
    ...
    startup that ``eco send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.eco/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.eco/config.yaml`` into
    ...
    intentionally reimplement the minimum needed here so ``eco send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.eco/.env and ~/.eco/config.yaml into os.environ so the
    ...
    if not target:
        print(
            "eco send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  eco send --to telegram \"hello\"\n"
            "  eco send --to discord:#ops --file report.md\n"
            "  eco send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "eco send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `eco send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform ECO "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.eco/.env + ~/.eco/config.yaml) — no LLM, "
            "no agent loop, no running gateway required for bot-token "
            "platforms like Telegram/Discord/Slack/Signal."
        ),
        epilog=(
            "Examples:\n"
            "  eco send --to telegram \"deploy finished\"\n"
            "  echo \"RAM 92%\" | eco send --to telegram:-1001234567890\n"
            "  eco send --to discord:#ops --file /tmp/report.md\n"
            "  eco send --to slack:#eng --subject \"[CI]\" --file build.log\n"
            "  eco send --list                  # all platforms\n"
            "  eco send --list telegram         # filter by platform\n"
            "\n"
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `eco send --list telegram`.",
    )
