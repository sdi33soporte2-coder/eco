"""Shell completion script generation for eco CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_eco_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# ECO Agent bash completion
# Add to ~/.bashrc:
#   eval "$(eco completion bash)"

_eco_profiles() {{
    local profiles_dir="$HOME/.eco/profiles"
...
_eco_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_eco_profiles)" -- "$cur"))
        return
    fi
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef eco
# ECO Agent zsh completion
# Add to ~/.zshrc:
#   eval "$(eco completion zsh)"

_eco_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.eco/profiles" ]]; then
        profiles+=("${{(@f)$(ls $HOME/.eco/profiles 2>/dev/null)}}")
    fi
...
_eco() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_eco_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# ECO Agent fish completion",
        "# Add to your config:",
        "#   eco completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __eco_profiles",
        "    echo default",
        "    if test -d $HOME/.eco/profiles",
        "        ls $HOME/.eco/profiles 2>/dev/null",
        "    end",
        "end",
        "",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c eco -f -s p -l profile"
        " -d 'Profile name' -xa '(__eco_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__eco_profiles)' -d 'Profile name'"
                )
