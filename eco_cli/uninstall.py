"""
ECO Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.eco/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove ECO PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing eco or eco PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# ECO Agent" comment and following line
                if '# ECO Agent' in line or '# eco' in line:
                    skip_next = True
                    continue
                if skip_next and ('eco' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing eco
                if 'eco' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the eco wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "eco",
        Path("/usr/local/bin/eco"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains eco_cli reference)
                content = wrapper.read_text()
                if 'eco_cli' in content or 'eco' in content:
                    wrapper.unlink()
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``eco gateway run`` processes
    ...
    try:
        from eco_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from eco_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``ECO_HOME`` and ``ECO_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\eco\git\cmd``, ``%LOCALAPPDATA%\eco\git\bin``,
#      ``%LOCALAPPDATA%\eco\git\usr\bin``, ``%LOCALAPPDATA%\eco\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\eco\git\`` and Node to
#      ``%LOCALAPPDATA%\eco\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``eco dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\eco\gateway-service\`` and sometimes
...
def _eco_path_markers(eco_home: Path) -> list[str]:
    """Path-entry substrings that identify ECO-owned User-PATH entries."""
    root = str(eco_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare eco install dir.
    markers = [root + "\\eco", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if ECO_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "eco".  We don't want to catch
    # unrelated entries like "chermes-foo" or "ephermeral", so we look for
    # backslash-eco as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(eco_home: Path) -> list[str]:
    """Strip ECO-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _eco_path_markers(eco_home)
            kept: list[str] = []
...
def remove_eco_env_vars_windows() -> list[str]:
    """Delete ECO_HOME and ECO_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("ECO_HOME", "ECO_GIT_BASH_PATH"):
                try:
...
def remove_portable_tooling_windows(eco_home: Path) -> list[Path]:
    """Delete PortableGit and Node installs the Windows installer created under
    ``%LOCALAPPDATA%\\eco\\``.  Only called on full uninstall; they're
...
def _is_default_eco_home(eco_home: Path) -> bool:
    """Return True when ``eco_home`` points at the default (non-profile) root."""
    try:
        from eco_constants import get_default_eco_root
        return eco_home.resolve() == get_default_eco_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its ECO_HOME directory.

    We shell out to ``eco -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current ECO_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m eco_cli.main` so we don't depend on a `eco`
    #    wrapper that may be half-removed mid-uninstall.
    eco_invocation = [_sys.executable, "-m", "eco_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                eco_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's ECO_HOME directory.
    try:
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.eco/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.eco/ for future reinstall
    """
    project_root = get_project_root()
    eco_home = get_eco_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie ECO_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_eco_home(eco_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ ECO Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {eco_home / 'config.yaml'}")
    print(f"  Secrets: {eco_home / '.env'}")
    print(f"  Data:    {eco_home / 'cron/'}, {eco_home / 'sessions/'}, {eco_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their ECO_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL ECO data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the ECO code but keep your configuration and data.")
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in eco_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\eco\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(eco_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No ECO-owned PATH entries in User environment")

        log_info("Removing ECO_HOME / ECO_GIT_BASH_PATH User env vars...")
        removed_env = remove_eco_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No ECO-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing eco command...")
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.eco/, just remove the eco subdir
            if eco_home in project_root.parents or project_root.parent == eco_home:
    ...
    #     under ECO_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(eco_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(eco_home)
    ...
    # 5. Optionally remove ~/.eco/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile ECO_HOME dirs live under
        ...
        try:
            if eco_home.exists():
                shutil.rmtree(eco_home)
                log_success(f"Removed {eco_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {eco_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {eco_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://raw.githubusercontent.com/NousResearch/hermes-agent-agent-agent/main/scripts/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://raw.githubusercontent.com/NousResearch/hermes-agent-agent-agent/main/scripts/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using ECO Agent! ⚕")
    print()
