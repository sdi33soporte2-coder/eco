"""Migrate ECO' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads ECO' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  ECO mcp_servers.<n>.command/args/env  → codex stdio transport
  ECO mcp_servers.<n>.url/headers       → codex streamable_http transport
  ECO mcp_servers.<n>.timeout           → codex tool_timeout_sec
  ECO mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  ECO-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. ECO' own
...
MIGRATION_MARKER = (
    "# managed by eco — `eco codex-runtime migrate` regenerates this section"
)
MIGRATION_END_MARKER = (
    "# end eco managed section"
)
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in ECO config.")
...
# ECO keys that codex's MCP schema doesn't support — dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    # ECO' sampling subsection — codex MCP has no equivalent
    "sampling",
}


def _translate_one_server(
    name: str, eco_cfg: dict
) -> tuple[Optional[dict], list[str]]:
    """Translate one ECO MCP server config to the codex inline-table dict
    ...
    has_command = bool(eco_cfg.get("command"))
    has_url = bool(eco_cfg.get("url"))
    ...
    if has_command:
        # Stdio transport
        out["command"] = str(eco_cfg["command"])
        args = eco_cfg.get("args") or []
        if args:
            out["args"] = [str(a) for a in args]
        env = eco_cfg.get("env") or {}
        if env:
        ...
        out["url"] = str(eco_cfg["url"])
        headers = eco_cfg.get("headers") or {}
        if headers:
            out["http_headers"] = {str(k): str(v) for k, v in headers.items()}
        # ECO' transport: sse hint is informational; codex auto-negotiates
        if eco_cfg.get("transport") == "sse":
            skipped.append("transport=sse (codex auto-negotiates)")
    else:
        return None, ["no command or url field"]

    # Timeouts
    if "timeout" in eco_cfg:
        try:
            out["tool_timeout_sec"] = float(eco_cfg["timeout"])
        except (TypeError, ValueError):
            skipped.append("timeout (not numeric)")
    if "connect_timeout" in eco_cfg:
        try:
            out["startup_timeout_sec"] = float(eco_cfg["connect_timeout"])
        except (TypeError, ValueError):
            skipped.append("connect_timeout (not numeric)")

    # Enabled flag (codex defaults to true so we only emit when explicitly false)
    if eco_cfg.get("enabled") is False:
        out["enabled"] = False
    ...
    for key in eco_cfg:
        ...
        elif key not in _KNOWN_ECO_KEYS:
            skipped.append(f"{key} (unknown ECO key)")
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (ECO_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by ECO)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert ECO' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before ECO' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior ECO
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="eco-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a ECO_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed eco-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.eco`` symlink that happens to live under ``/private/var/folders``
...
def _build_eco_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches ECO' own
    ...
    so a eco installed under /opt/, /usr/local/, or a venv all work.
    ECO_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # ECO_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_eco_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever ECO_HOME its launcher sets
    ...
    # — that would override the launcher's ECO_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("ECO_HOME", tmp_path) would
    ...
    eco_home = os.environ.get("ECO_HOME") or ""
    if eco_home and _looks_like_test_tempdir(eco_home):
        eco_home = ""
    if eco_home:
        env["ECO_HOME"] = eco_home
    # PYTHONPATH passes through so a worktree-launched eco finds the
    ...
    env["ECO_QUIET"] = "1"
    env["ECO_REDACT_SECRETS"] = env.get("ECO_REDACT_SECRETS", "true")

    out: dict[str, Any] = {
        "command": sys.executable,
        "args": ["-m", "agent.transports.eco_tools_mcp_server"],
    }
    if env:
        out["env"] = env
...
def migrate(
    ...
    *,
    ...
    expose_eco_tools: bool = True,
) -> MigrationReport:
    """Translate ECO mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        eco_config: full ~/.eco/config.yaml dict
    ...
    eco_servers = (eco_config or {}).get("mcp_servers") or {}
    if not isinstance(eco_servers, dict):
        report.errors.append(
            "mcp_servers in ECO config is not a dict; cannot migrate."
        )
        return report

    translated: dict[str, dict] = {}
    for name, cfg in eco_servers.items():
    ...
    # Inject ECO' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into ECO for the tools codex
    # doesn't ship with — web_search, browser_*, delegate_task, vision,
    # memory, skills, session_search, image_generate, text_to_speech.
    # The server itself is agent/transports/eco_tools_mcp_server.py
    # and is launched on demand by codex (stdio MCP).
    if expose_eco_tools:
        translated["eco-tools"] = _build_eco_tools_mcp_entry()
        if "eco-tools" not in report.migrated:
            report.migrated.append("eco-tools")
