...
# xAI's public API is OpenAI-compatible for the endpoints ECO commonly
# uses. The Responses endpoint is included because ECO' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via ECO-managed OAuth credentials."""

    auth_hint = "eco auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`eco auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`eco auth reset xai-oauth` or re-authenticate with "
                    "`eco auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `eco auth add xai-oauth --type oauth`."
            )
