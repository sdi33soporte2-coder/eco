"""
Dump command for eco CLI.

Outputs a compact, plain-text summary of the user's ECO setup
...
from eco_cli.config import get_eco_home, get_env_path, get_project_root, load_config
...
def _get_git_commit(project_root: Path) -> str:
    ...
    ``ECO_GIT_SHA`` build-arg (see ``eco_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``eco dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from eco_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(eco_home: Path) -> int:
...
def _cron_summary(eco_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = eco_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"eco_home:      {display_eco_home()}")
    ...
    toolsets = config.get("toolsets", ["eco-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(eco_home)}")
    lines.append(f"  skills:             {_count_skills(eco_home)}")
