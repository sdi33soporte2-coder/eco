"""
eco fallback — manage the fallback provider chain.
...
Subcommands:
  eco fallback [list]   Show the current fallback chain (default when no subcommand)
  eco fallback add      Pick provider + model via the same picker as `eco model`,
                           then append the selection to the chain
  eco fallback remove   Pick an entry to delete from the chain
  eco fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.eco/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from eco_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `eco model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  eco fallback add")
        print()
        return
    ...
    print("  Docs: https://eco.nousresearch.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `eco model`, then append the selection to the chain."""
    from eco_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `eco model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `eco fallback list` to view, or `eco fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``eco fallback [subcommand]``."""
