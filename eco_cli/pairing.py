...
Usage:
    eco pairing list              # Show all pending + approved users
    eco pairing approve <platform> <code>  # Approve a pairing code
    eco pairing revoke <platform> <user_id> # Revoke user access
    eco pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle eco pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: eco pairing {list|approve|revoke|clear-pending}")
        print("Run 'eco pairing --help' for details.")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.eco/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'eco pairing list' to see pending codes.\n")
