"""`eco checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.eco/checkpoints/``.  Actions:

    eco checkpoints               # same as `status`
    eco checkpoints status        # total size, project count, breakdown
    eco checkpoints list          # per-project checkpoint counts + workdir
    eco checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    eco checkpoints clear [-f]    # nuke the entire base (asks first)
    eco checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    eco checkpoints
    eco checkpoints prune --retention-days 3 --max-size-mb 200
    eco checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: eco checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``eco checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `eco checkpoints` → status
