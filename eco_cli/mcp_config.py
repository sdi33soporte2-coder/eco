"""
MCP Server Management CLI — ``eco mcp`` subcommand.

Implements ``eco mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.eco/config.yaml under the ``mcp_servers`` key.
"""
...
from eco_cli.config import (
    ...
    get_env_value,
    save_env_value,
    get_eco_home,  # noqa: F401 — used by test mocks
)
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from eco_cli.cli_output import prompt as _shared_prompt
...
def cmd_mcp_add(args):
    ...
    # eco_cli/main.py for why the dest is renamed.
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  eco mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  eco mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  eco mcp add myserver --preset mypreset')
        return
            ...
            if auth_type == "header" or not auth_type:
                ...
                if existing_key:
                    ...
                    if api_key:
                        ...
                        _success(f"Saved to {display_eco_home()}/.env as {env_key}")
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            _info("Fix the issue, then: eco mcp test " + name)
        return
    ...
    print()
    _success(f"Saved '{name}' to {display_eco_home()}/config.yaml ({tool_count}/{total} tools enabled)")
...
def cmd_mcp_remove(args):
    ...
    # earlier `eco mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  eco mcp add <name> --url <endpoint>')
        _info('  eco mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def cmd_mcp_login(args):
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `eco mcp remove` + `eco mcp add` to reconfigure auth.")
        return
...
def cmd_mcp_configure(args):
    ...
    if not _sys.stdin.isatty():
        print("Error: 'eco mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``eco mcp`` subcommands."""
    ...
    if action == "picker":
        from eco_cli.mcp_picker import run_picker
        run_picker()
        return
    if action == "catalog":
        from eco_cli.mcp_picker import show_catalog
        show_catalog()
        return
    if action == "install":
        from eco_cli.mcp_picker import install_by_name
        import sys as _sys
    ...
    if handler:
        ...
        # "try enabling and it flows you into setup" UX matching `eco plugin`.
        from eco_cli.mcp_picker import run_picker
        run_picker()
        print(color("  Commands:", Colors.CYAN))
        _info("eco mcp                                    Open the catalog picker (default)")
        _info("eco mcp catalog                            List Nous-approved MCPs")
        _info("eco mcp install <name>                     Install a catalog MCP")
        _info("eco mcp serve                              Run as MCP server")
        _info("eco mcp add <name> --url <endpoint>        Add a custom MCP server")
        _info("eco mcp add <name> --command <cmd>         Add a stdio server")
        _info("eco mcp add <name> --preset <preset>       Add from a known preset")
        _info("eco mcp remove <name>                      Remove a server")
        _info("eco mcp list                               List configured servers")
        _info("eco mcp test <name>                        Test connection")
        _info("eco mcp configure <name>                   Toggle tools")
        _info("eco mcp login <name>                       Re-authenticate OAuth")
        print()
