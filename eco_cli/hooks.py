"""eco hooks — inspect and manage shell-script hooks.

Usage::

    eco hooks list
    eco hooks test <event> [--for-tool X] [--payload-file F]
    eco hooks revoke <command>
    eco hooks doctor

Consent records live under ``~/.eco/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.eco/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``eco hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: eco hooks {list|test|revoke|doctor}")
        print("Run 'eco hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.eco/config.yaml.")
        print("See `eco hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `eco hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `eco hooks test` and `eco hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (ECO wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `eco hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `eco hooks doctor` would execute
