"""``eco debug`` debug tools for ECO Agent.

Currently supports:
    eco debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.eco/logs/*.log`` are not leaked into
...
_REDACTION_BANNER = (
    "[eco debug share: log content redacted at upload time. "
...
def _pending_file() -> Path:
    """Path to ``~/.eco/pastes/pending.json``.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``eco debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``eco debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
⚠️  This will upload the following to a public paste service:
  • System info (OS, Python version, ECO version, provider, which API keys
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `eco debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "eco/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``eco debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.eco/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``eco debug share`` also runs an opportunistic sweep as a fallback
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from eco_cli.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_eco_home() / "logs" / filename) if filename else None
...
def _capture_dump() -> str:
    """Run ``eco dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``eco dump`` is run
        internally.
...
def run_debug_share(args):
    ...
    if redact:
        logger.info(
            "eco debug share: applied force-mode redaction to log snapshots before upload"
        )
    ...
    print(f"To delete now:  eco debug delete <url>")

    print(f"\nShare these links with the ECO team for support.")
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: eco debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'eco debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``eco debug`` call.
    ...
    # best-effort — any failure is swallowed so ``eco debug`` stays
