...
configured for "cli" in `eco tools`.
Rules / memory / AGENTS.md / preloaded skills = same as a normal chat turn.
Approvals = auto-bypassed (ECO_YOLO_MODE=1 is set for the call).
Working directory = the user's CWD (AGENTS.md etc. resolve from there as usual).

Model / provider selection mirrors `eco chat`:
...
def _validate_explicit_toolsets(toolsets: object = None) -> tuple[list[str] | None, str | None]:
    ...
    try:
        ...
        return None, f"eco -z: failed to validate --toolsets: {exc}\n"
    ...
    if any(name in {"all", "*"} for name in built_in):
        ...
        if ignored:
            sys.stderr.write(
                "eco -z: --toolsets all enables every toolset; "
    ...
    if unknown:
        sys.stderr.write(f"eco -z: ignoring unknown --toolsets entries: {', '.join(unknown)}\n")
    if disabled:
        sys.stderr.write(
            "eco -z: ignoring disabled MCP servers (set enabled: true in config.yaml to use): "
            f"{', '.join(disabled)}\n"
        )

    if not valid:
        return None, "eco -z: --toolsets did not contain any valid toolsets.\n"
...
def run_oneshot(
    ...
    Args:
        ...
        model: Optional model override. Falls back to ECO_INFERENCE_MODEL
    ...
    env_model_early = os.getenv("ECO_INFERENCE_MODEL", "").strip()
    if provider and not ((model or "").strip() or env_model_early):
        sys.stderr.write(
            "eco -z: --provider requires --model (or ECO_INFERENCE_MODEL). "
...
def _create_session_db_for_oneshot():
    """Best-effort SessionDB for ``eco -z`` / oneshot mode.
...
def _run_agent(
    ...
    # Imports are local so they don't run when eco is invoked for
    # other commands (keeps top-level CLI startup cheap).
    from eco_cli.config import load_config
    from eco_cli.models import detect_provider_for_model
    ...
    env_model = os.getenv("ECO_INFERENCE_MODEL", "").strip()
    ...
    if effective_provider is None and (model or env_model):
        ...
        if explicit_model:
            ...
            try:
                from eco_cli import model_switch as _ms
            ...
            if direct is not None:
                ...
                current_provider = (
                    cfg_provider
                    or os.getenv("ECO_INFERENCE_PROVIDER", "").strip().lower()
                    or "auto"
                )
    ...
    agent = AIAgent(
        ...
        #                ECO_INTERACTIVE which we never set
        #   - shell-hook approval → auto-approved via ECO_ACCEPT_HOOKS=1
        #                (set above); also falls back to deny on non-tty
        #   - dangerous-command approval → bypassed via ECO_YOLO_MODE=1
